...
reviewed and confirmed in full: 2026-09-03.
 ...
 8. **Deletion is the operator's.** Quarantine by moving into a `_to_delete/`
    or clearly-named superseded folder, then say which files moved there.
 9. **Never handle or enter credentials or API keys.** `CDS_API_KEY` and the
    rest live in `SSI Index/SSI_v4_2 Italy Pilot/pipeline-v4.2/.env`, are read
...
- Audit reports at `$SSI_AUDIT_DIR/normalise_bands_audit_*.json` (per-run consolidated) + `fix_wave4_r_base_audit_*.json` (per-country). **Location changed 21 September 2026** — was loose in `~/`; now `--audit-dir` > `$SSI_AUDIT_DIR` > `~/ssi-audit-trail/`, resolved once in `scripts/pipeline/utils/audit_dir.py`.
...
- Audit reports at `$SSI_AUDIT_DIR/catchment_population_audit_*.json` (per-run consolidated; see `scripts/pipeline/utils/audit_dir.py` — was loose in `~/` before 21 September 2026)
...
- AFTER: 30 countries carry real Niva-derived values in [0, 1] with meaningful per-substation variation; 9 countries carry pre-existing genuine distributions in their native ranges (Task #450 SYSTEMIC will normalize these separately).
  - **CORRECTED 21 September 2026 — this claim was false when written.** 90,476 records across 18 countries held the ingestion neutral default `0.5` while carrying `_migration_score_source: NIVA_2023_20YR_SUM_v4_2_task_452`. The NIVA raster contains no zero pixel, and `0.5 + 0.5*tanh(raw/200)` returns 0.5 only at `raw == 0`, so the instrument cannot emit 0.5: all 1,922,207 finite pixels were tested and none does. portugal 13,517/13,563 and spain 12,008/12,437 were at the typed default, not at "real Niva-derived values". Repaired 21 September 2026 (39 countries, 621,056 measured / 1,048 Convention #56 nulls / 0 at 0.5). See `doctrine/FINDING_the_migration_marker_claims_a_source_that_cannot_have_produced_it.md`.
...
- Audit reports at `$SSI_AUDIT_DIR/migration_score_audit_*.json` (per-run consolidated; see `scripts/pipeline/utils/audit_dir.py` — was loose in `~/` before 21 September 2026)
...
- Consolidated audit report `$SSI_AUDIT_DIR/migration_score_semantic_normalise_audit_*.json` (see `scripts/pipeline/utils/audit_dir.py` — was loose in `~/` before 21 September 2026)
