...
- **Task #501 candidate — V_socio semantic-scale bridge (Wave 4 majors fleet-uniform closure)** — audit landed 24 July 2026. Empirical scope: **8 Wave 4 majors with fleet-uniform national-scalar V_socio signature** (France 195,569 subs @ 0.1867 · Germany 187,714 @ 0.1867 · Italy 51,910 @ 0.7275 · Japan 7,073 @ 0.6233 · Portugal 13,977 @ 0.4737 · Spain 30,222 @ 0.3993 · Sweden 11,399 @ 0.1867 · US 101,594 @ 0.1800) = **199,458 substations affected**. Also flagged: Turkey 4,001 subs / 6 unique values (partial-fleet-uniform post-2021-reform pattern). Envelope check GREEN: 0 countries with V_socio outside [0, 1] envelope — V_socio drift is fleet-uniform NOT semantic-scale (distinct from Task #450 migration_score class). **Correct fix pattern**: Task #452/#453 NUTS-3 polygon spatial-join per-country (compute per-region V_socio from CSV lookup replacing fleet-uniform national scalar) — Task #450 min-max rescale does NOT apply (would degenerate on min == max). Estimated scope: 8-16 engineer-hours across 8 countries × Eurostat GISCO NUTS-3 shapefile reuse (already downloaded for Task #453/#454) + agency_regional_socioeconomic CSV scaffolding for non-EU (US + Japan) + shared utility extension (`scripts/pipeline/enrichment/v_socio_polygon_backfill.py` following Task #453 pattern). Deferred to follow-on session.
- Task #454b Greece **BLOCKED** — empirical audit landed 24 July 2026. 163 v43 subs · 100% no-province · 100% no-gdp · **CSV MISSING** (both `eurostat_nuts3_socioeconomic.csv` and `agency_regional_socioeconomic.csv`). Blocker: operator-sourced ELSTAT NUTS-3 data required to scaffold Greece CSV (EL30..EL65 codes). Once CSV lands, execution is a Task #454 shared-utility replay (~30 minutes).
- Task #454c Greenland **DEFERRED** — empirical audit landed 24 July 2026. 6 v43 subs · 100% no-province · GDP **already populated** (€32,310 uniform national-scalar fallback pathway). Polygon join marginal value since GDP already populated; deferred to lowest priority.
