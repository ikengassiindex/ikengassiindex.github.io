...
def _series(n_days, base, amp, spikes=(), noise=1.5):
    ...
    return (v + 273.15).astype("float32").reshape(-1, 1, 1)
...
def test_a_run_shorter_than_three_days_is_not_a_heat_wave():
    ...
    d2, _ = i3.heatwave_field(two, doy)
    d3, _ = i3.heatwave_field(three, doy)
    assert d2[0, 0] == 0.0, "a 2-day spike must not count as a heat wave"
    assert d3[0, 0] > 0.0, "a 3-day spike must count"


def test_more_excess_gives_a_higher_metric_not_lower():
    """I3 carries RISK. Unlike I4/I6 it is NOT inverted."""
    n = int(365.25 * 5); doy = np.arange(n) % 366
    a, _ = i3.heatwave_field(_series(n, 15, 10, _summer_spikes(5, 3)), doy)
    b, _ = i3.heatwave_field(_series(n, 15, 10, _summer_spikes(5, 15)), doy)
    assert b[0, 0] > a[0, 0], f"fiercer heat scored lower: {b[0,0]} vs {a[0,0]}"
    lo = i3.method_b(float(a[0, 0]), 0.0, float(b[0, 0]))
    hi = i3.method_b(float(b[0, 0]), 0.0, float(b[0, 0]))
...
def test_the_baseline_tracks_the_season_not_the_year():
    ...
    d, _ = i3.heatwave_field(_series(n, 15, 15, winter), doy)
    assert d[0, 0] > 0.0, "a winter heat anomaly must register against a seasonal baseline"
...
def test_magnitude_separates_where_frequency_cannot():
    ...
    n = int(365.25 * 5); doy = np.arange(n) % 366
    calm, _ = i3.heatwave_field(_series(n, 15, 10, _summer_spikes(5, 2)), doy)
    wild, _ = i3.heatwave_field(_series(n, 15, 10, _summer_spikes(5, 12)), doy)
    assert wild[0, 0] > calm[0, 0], "magnitude must separate these"
    _, dc = i3.heatwave_field(_series(n, 15, 10, _summer_spikes(5, 2)), doy)
    _, dw = i3.heatwave_field(_series(n, 15, 10, _summer_spikes(5, 12)), doy)
    assert abs(float(dw[0, 0]) - float(dc[0, 0])) < float(wild[0, 0]), \
...
def test_an_event_that_recurs_identically_every_year_IS_the_climatology():
    ...
    d, _ = i3.heatwave_field(_series(n, 15, 10, fixed, noise=0.0), doy)
    assert d[0, 0] == 0.0
...
def test_an_all_nan_cell_never_becomes_a_real_zero():
    ...
    sea = np.full((366, 1, 1), np.nan, dtype="float32")
    d, _ = i3.heatwave_field(sea, doy)
    assert d[0, 0] == 0.0, "sum of nothing is 0 — which is exactly the trap"
    valid = np.isfinite(sea).any(axis=0)
    assert not valid[0, 0], "the validity mask, not the value, is what must gate it"
