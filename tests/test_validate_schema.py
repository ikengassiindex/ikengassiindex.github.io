...
class TestPipelinePhase2b:
    ...
    def test_12_pipeline_phase_2b_passes_italy(self):
        ...
        # for PR-7). NOTICE: lines are also excluded — they state what the
        # validator DID rather than that something may be wrong, and the
        # validator marks them explicitly for exactly this reason. Anything
        # else on the warnings channel is a real warning and fails here.
        non_provenance = [w for w in warnings
                          if "PR-3 PROVENANCE" not in w and not w.startswith("NOTICE:")]
