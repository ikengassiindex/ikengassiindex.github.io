...
def audit_country(slug: str, fr_map: dict, today: datetime.date,
    ...
    #
    # Pre-launch JSONs are placeholder/scaffold data that will be
    # regenerated by first_refresh. Any schema warnings on them are
    # expected, self-resolving noise — downgrade to info-level so the
    # signal-from-noise of the warning bucket stays high.
    if result["pre_launch"]:
        ...
        for f in result["schema_findings"]:
            if f.get("severity") in ("warning", "critical"):
                f["severity"] = "info"
                f["label"] = f"{f['label']} [pre-launch placeholder]"
