...
(function () {
  ...
  function safeRequireRegistration(cb) {
    // nav.js exposes this as a global. It now always returns false: the
    // dataset-request panel is shown and the caller does not proceed.
    //
    // FAIL CLOSED. This used to `return true` when the global was missing,
    // so a failed nav.js load silently opened the four download cards. The
    // operator's instruction of 24 September is that the dataset is obtained
    // by request; a fallback that grants it when a script fails to load is
    // the opposite of that, and a gate whose failure mode is "open" is not a
    // gate. If nav.js is absent the panel cannot be drawn, so the honest
    // response is to do nothing rather than to generate the file.
