"""
SSI Pipeline — Netherlands v4.23 federation merger.
...
Merger operations (Option A per pre-flight):
  ...
  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     ...
     Netherlands baseline: 0% owner tagged. Expected post-merge: 90-100%.
...
SSI_DATA_JSON = REPO_ROOT / "netherlands" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "netherlands" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "netherlands" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "NL_v43_") -> str:
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (netherlands/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="NL_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Netherlands (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Netherlands v4.23 federation merger")
