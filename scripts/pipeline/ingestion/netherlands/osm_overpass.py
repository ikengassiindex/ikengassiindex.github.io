"""
SSI Pipeline — Netherlands L1 connector: OSM Overpass API.
...
DIFFERENCE FROM BELGIUM CONNECTOR:
  - COMPACT SCALE — Netherlands is ~42k km² (~1/250 Australia). Single-query
...
# ── Query builders ──────────────────────────────────────────────────────
_NL_AREA_HEADER = 'area["ISO3166-1"="NL"]->.nl'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_NL_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.nl);'
        f'out center tags;'
    )
...
def _query_lines() -> str:
    ...
    Single-query safe for small country — Netherlands bbox ~42k km².
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_NL_AREA_HEADER};'
        f'(way["power"="line"](area.nl);'
        f'way["power"="cable"](area.nl);'
        f'way["power"="minor_line"](area.nl););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    if op_tag:
        ...
        owner, owner_provenance = resolve_owner_from_region_jurisdiction(
            lat, lon, voltage_kv, osm_province=osm_region
        )
...
def fetch(
    *,
    ...
    """Fetch OSM Netherlands substations + lines (single-query per element type).
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Netherlands polygon "
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass NL L1 connector")
