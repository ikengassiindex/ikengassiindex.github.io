"""Italy P34 Wave 4 — Merge OSM subs + lines into canonical files.
...
INHERITED (canonical template post-Portugal):
  ...
  - Sub ID assignment (10-digit format starting 7000000000+)
  - Line ID assignment (integer 50000000+ Italy convention)
  ...
  - Bi-directional Option B pattern (Italy-specific voltages):
    * Lines: minor_line → 20 kV MV; cable → 20 kV MV; line → 132 kV
      Italian subT (dominant vs Portugal 60 kV, Nordic 130 kV)
    * Subs: substation=transmission → 220 kV Terna; substation=
      ...
      traction → 3 kV DC RFI rail; power=substation generic → 20 kV
  - Discipline #36 cross-border filter (5.0 km tolerance)
  - Discipline #41 line-substation parity audit
  - Convention #78 BINDING 16th enforcement counter (8-language)
  - Convention #78 §4bis.5 11th (Milan) + 12th (Rome) DUAL enforcement
  - Convention #56 partial-fetch preservation
  - Convention #80 grid-geo sharding integration

Emits:
  italy/ssi-data.json (wrapped schema, backed up first)
  italy/grid-geo.json (canonical compact {s, l, a})
  scripts/pipeline/data/italy/v4_23-ingestion-audit-italy-merge-sidecar.json
"""
...
from scripts.pipeline.ingestion.italy._base import (  # noqa: E402
    ITALY_BBOX,
    alias_hit_count,
...
def _in_italy_bbox(lat: float, lon: float) -> bool:
    return (
        ITALY_BBOX["lat_min"] <= lat <= ITALY_BBOX["lat_max"]
        and ITALY_BBOX["lon_min"] <= lon <= ITALY_BBOX["lon_max"]
    )
...
def _process_subs(
    ...
    # Italy convention: sub IDs start at 7000000000
    next_sub_id = 7_000_000_000
    ...
    for element in raw_subs.get("elements", []):
        ...
        # Portugal P33 Option B extended to subs (Italy-specific defaults):
        #   substation=transmission → 220 kV Terna national TSO backbone
        #   substation=distribution → 20 kV E-Distribuzione MV (Italian
        #     standard MV distribution voltage — differs from Portugal
        #     30 kV E-REDES; Italian E-Distribuzione uses 20 kV)
        #   substation=minor_distribution → 15 kV E-Distribuzione MV rural
        #   substation=traction → 3 kV RFI rail DC (Italian rail standard
        #     — differs from Portugal 25 kV AC; Italy uses 3 kV DC classic
        #     + 25 kV AC TAV high-speed on select lines)
        #   power=substation generic → 20 kV E-Distribuzione MV default
        if voltage_kv is None:
            ...
            if substation_class == "transmission":
                ...
                voltage_kv = 20.0  # Italian MV distribution standard
                voltage_inferred = True
            elif substation_class == "minor_distribution":
                voltage_kv = 15.0
                voltage_inferred = True
            elif substation_class == "traction":
                voltage_kv = 3.0  # Italian rail 3 kV DC classic
                voltage_inferred = True
...
# ─────────────────────────────────────────────────────────────
# Line processing — Sweden P32 Option B + Italy voltage inference
# ─────────────────────────────────────────────────────────────
...
def _process_lines(
    ...
    # Italy convention: line IDs start at 50000000
    next_line_id = 50_000_000
    ...
    for element in raw_lines.get("elements", []):
        ...
        voltage_inferred = False
        if voltage_kv is None:
            # ITALY-SPECIFIC voltage inference:
            #   minor_line → 20 kV MV rural distribution
            #   cable no-voltage → 20 kV MV underground urban
            #   line no-voltage → 132 kV Italian subtransmission
            # (Italian dominant subT voltage 132 kV — differs from
            # Portugal 60 kV, Nordic Sweden 130 kV)
...
def _build_sidecar(
    ...
    milan_hits = (
        resolution_counts.get("milan_a2a_unareti_geofence", 0)
        + resolution_counts.get("milan_e_distribuzione_geofence", 0)
    )
    rome_hits = (
        resolution_counts.get("rome_acea_geofence", 0)
        + resolution_counts.get("rome_e_distribuzione_geofence", 0)
    )
    return {
        "workstream": "SSI v4.23 Wave 4 — Italy P34",
        "priority": "P34",
        "wave": 4,
        "smallest_first_rank": 4,
        "architecture": "wave_4_portugal_p33_bi_directional_option_b_inheritance_terna_7_zone",
        "scope": "WAVE_4_MEDITERRANEAN_MV_INHERITANCE_FROM_PORTUGAL_P33_BI_DIRECTIONAL",
        "delta": {
        ...
        "convention_78_binding_16th_enforcement": {
            "cohort_wide_enforcement_number": 16,
            "script_class": "ITALIAN_PLUS_ENGLISH_PLUS_GERMAN_PLUS_FRENCH_PLUS_SLOVENIAN_PLUS_LADIN_PLUS_FRIULIAN_PLUS_SARDINIAN_8_LANGUAGE",
            "cohort_wide_signature": "HIGHEST_LANGUAGE_COUNT_COHORT_WIDE_8_LANGUAGES",
            "alias_hits": alias_hit_count(),
        },
        "convention_78_section_4bis_5_dual_enforcement": {
            "milan_11th_enforcement": {
                "cohort_wide_enforcement_number": 11,
                "geofence_architecture": "milan_2_way_a2a_unareti_plus_e_distribuzione",
                "milan_a2a_unareti_hits": resolution_counts.get(
                    "milan_a2a_unareti_geofence", 0
                ),
                "milan_e_distribuzione_hits": resolution_counts.get(
                    "milan_e_distribuzione_geofence", 0
                ),
                "total_milan_hits": milan_hits,
            },
            "rome_12th_enforcement": {
                "cohort_wide_enforcement_number": 12,
                "geofence_architecture": "rome_2_way_acea_plus_e_distribuzione",
                "rome_acea_hits": resolution_counts.get("rome_acea_geofence", 0),
                "rome_e_distribuzione_hits": resolution_counts.get(
                    "rome_e_distribuzione_geofence", 0
                ),
                "total_rome_hits": rome_hits,
            },
            "total_4bis_5_hits_italy": milan_hits + rome_hits,
        },
        ...
        "discipline_41_parity": {
            ...
            "baseline_pre_enhancement": 3.31,
        },
        "portugal_p33_bi_directional_option_b_inheritance": {
            "lines_side_active": True,
            "subs_side_active": True,
            "italy_specific_line_voltage_inference_kv": {
                "minor_line": 20.0,
                "cable_no_voltage": 20.0,
                "line_no_voltage": 132.0,  # Italian subT dominant
            },
            "italy_specific_sub_voltage_inference_kv": {
                "substation_transmission": 220.0,  # Terna national TSO
                "substation_distribution": 20.0,  # E-Distribuzione MV standard
                "substation_minor_distribution": 15.0,  # E-Distribuzione MV rural
                "substation_traction": 3.0,  # RFI 3 kV DC classic
                "power_substation_generic": 20.0,  # E-Distribuzione MV default
            },
            ...
            "convention_56_visibly_honest": (
                ...
                "compact flag. Italian line-side uses 132 kV subT (vs "
                "Portugal 60 kV, Nordic 130 kV). Italian sub-side uses "
                "20 kV MV E-Distribuzione (vs Portugal 30 kV E-REDES)."
            ),
        },
        ...
        "architectural_first_instances": [
            "🇮🇹 FIRST Mediterranean Wave 4 country enhancement",
            "🌍 HIGHEST cohort-wide 8-language Convention #78 BINDING (Italian+English+German+French+Slovenian+Ladin+Friulian+Sardinian)",
            "Convention #78 §4bis.5 11TH enforcement (Milan A2A/Unareti + E-Distribuzione)",
            "Convention #78 §4bis.5 12TH enforcement (Rome ACEA + E-Distribuzione)",
            "Post-2021 Terna 7-zone bidding architecture FIRST cohort-wide",
            "3 subsea HVDC island interconnectors preserved (SAPEI + Sorgente-Rizziconi + Malta)",
            "FIRST Alpine + Mediterranean dual coastline pattern",
            "6th most seismic OECD (L'Aquila + Amatrice + Norcia + Emilia + Etna/Vesuvio/Stromboli active volcanism)",
            "FIRST Mediterranean OECD offshore wind (Beleolico Taranto 2022)",
            "Portugal P33 bi-directional Option B pattern FIRST inheritance beyond Iberian (Mediterranean scale)",
        ],
    }
...
def main() -> None:
    print("═" * 70)
    print("Italy P34 Wave 4 — Merge OSM into ssi-data.json + grid-geo.json")
    print("🇮🇹 Post-2021 Terna 7-zone + Portugal P33 bi-directional Option B")
    print("═" * 70)

    country_dir = Path("italy")
    ...
    shutil.copy2(ssi_data_path, ssi_data_path.with_suffix(".json.pre_p34.bak"))
    shutil.copy2(grid_geo_path, grid_geo_path.with_suffix(".json.pre_p34.bak"))

    new_ssi = {
        "substations": subs_list,
        "_v4_23_ingestion_pass": "italy_p34_wave_4",
    ...
    sidecar_dir = Path("scripts/pipeline/data/italy")
    sidecar_dir.mkdir(parents=True, exist_ok=True)
    sidecar_path = sidecar_dir / "v4_23-ingestion-audit-italy-merge-sidecar.json"
    ...
    print()
    print("═" * 70)
    print("✓ Italy P34 merge complete")
    print(
    ...
    print(f"  Convention #78 BINDING 16th alias hits: {alias_hit_count():,}")
    milan_hits = (
        resolution_counts.get("milan_a2a_unareti_geofence", 0)
        + resolution_counts.get("milan_e_distribuzione_geofence", 0)
    )
    ...
    print(f"  Milan §4bis.5 11th enforcement hits: {milan_hits:,}")
    print(f"  Rome §4bis.5 12th enforcement hits: {rome_hits:,}")
    print(
