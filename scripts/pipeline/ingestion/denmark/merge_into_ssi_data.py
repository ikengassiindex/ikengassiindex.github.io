"""
SSI Pipeline — Denmark v4.23 federation merger.

Merges OSM Overpass L1 ingestion into denmark/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Danish + English + minimal German
     Schleswig cohabitation + Danish diacritic macron composition (æ ø å)
     + predecessor rebrand cascades (DONG→Ørsted 2017 + SEAS-NVE→Cerius
     2020 + Eltra+Elkraft→Energinet 2005 + TRE-FOR→Trefor 2011+EWII 2016
     + Norlys 2019 merger) + offshore wind generation-vs-distribution
     separation Unicode NFC alias normalisation per Convention #78
     BINDING 10th DECADE MILESTONE) OR 6-layer multi-DSO resolver per
     _base.py::resolve_owner_from_region_jurisdiction (Layer 1
     Energinet TSO ≥132 kV OR Layer 2 Banedanmark 25 kV AC OR Layer 3a
     offshore wind name-match OR Layer 3b §4bis.5 Copenhagen
     metropolitan Radius geofence 6TH COHORT-WIDE ENFORCEMENT OR
     Layer 4 industrial captive OR Layer 5 region → dominant DSO OR
     Layer 6 Energinet catch-all). Denmark baseline: 0.0% owner tagged
     (CLEAN SLATE PATTERN — Ireland/NZ analog).
     Expected post-merge: ~100% (Nordic offshore-wind multi-DSO).

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 99%+ populated so few
     fills expected — 400/150/132/60/50/30/20/15/10 kV tiers).
  ...
  4b. Step 4b — Retroactive owner attribution on baseline subs without
      ...
      6-layer multi-DSO resolver to close 100% owner coverage.
      Expected MODERATE cohort-wide retroactive diversity (8-12 owner
      classes — 4 major DSOs + Energinet + Banedanmark + offshore wind
      + captives).
...
OSM tag (including Danish legal-forms A/S/ApS/AmbA/FmbA, Danish
diacritic composition æ ø å, offshore wind generation-vs-distribution
separation Ørsted/Vattenfall NOT DSO flag-routing, DONG→Ørsted 2017 +
SEAS-NVE→Cerius 2020 + Eltra+Elkraft→Energinet 2005 + TRE-FOR→Trefor
2011+EWII 2016 + Norlys 2019 merger predecessor cascades, and
municipal energy department pre-1993 corporatization predecessors) in
...
SSI_DATA_JSON = REPO_ROOT / "denmark" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "denmark" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "denmark" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "DK_v43_") -> str:
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            v43_prov[SOURCE_ID] = {
                ...
                "osm_name": osm_name,
                "osm_name_da": ov.raw_attributes.get("osm_name_da"),
                "osm_name_en": ov.raw_attributes.get("osm_name_en"),
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply 6-layer multi-DSO resolver to baseline subs without OSM match.

    Pattern surfaced in Costa Rica Priority 13: when OSM sub count is
    smaller than baseline OR spatial coverage differs, the OSM-driven
    merge leaves unmatched baseline subs without owner. Applying the
    same resolver retroactively closes the gap using the same
    architectural rules. Provenance tagged with 'retroactive_' prefix
    for audit trail distinction.

    Denmark Nordic offshore-wind multi-DSO retroactive attribution:
    - Layer 1 Energinet TSO ≥132 kV (rare — most baseline subs are
      distribution tier 60/50/30/20/15/10 kV)
    - Layer 2 Banedanmark 25 kV AC rail traction (name-match)
    - Layer 3a Offshore wind farm (name-match — Horns Rev + Anholt +
      Kriegers Flak + Vesterhav + Rødsand + Nysted + Middelgrunden)
    - Layer 3b §4bis.5 Copenhagen metropolitan Radius geofence (6TH
      COHORT-WIDE ENFORCEMENT)
    - Layer 4 Industrial captive (Novo Nordisk + Arla + Grundfos + LEGO
      + Aalborg Portland + Danish Crown + Mærsk)
    - Layer 5 Region → dominant DSO (DOMINANT retroactive for 5 admin
      regions × 4 major DSOs)
    - Layer 6 Energinet catch-all (safety net for edge cases)

    Every unmatched baseline sub gets attributed (100% coverage
    post-4b). MODERATE cohort-wide retroactive diversity expected
    (8-12 distinct owner classes).
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        # Denmark 5 admin regions (populated from baseline)
        admin_code = s.get("region") or s.get("province")
        name = s.get("name") or s.get("operator_station_name")
        owner, provenance = resolve_owner_from_region_jurisdiction(
            v, float(lat), float(lon), admin_code=admin_code, name=name
        )
        if owner:
            s["owner"] = owner
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "(500m proximity miss). Same 6-layer multi-DSO resolver as OSM "
                    "path (Layer 1 Energinet TSO ≥132 kV OR Layer 2 Banedanmark "
                    "25 kV AC traction OR Layer 3a offshore wind name-match OR "
                    "Layer 3b §4bis.5 Copenhagen Radius metropolitan geofence 6TH "
                    "COHORT-WIDE ENFORCEMENT OR Layer 4 industrial captive OR "
                    "Layer 5 region → dominant DSO OR Layer 6 Energinet catch-all). "
                    "Codified in Costa Rica Priority 13 closure; Denmark Nordic "
                    "offshore-wind cohort with 4 major DSOs + Convention #78 "
                    "BINDING 10th DECADE MILESTONE."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (denmark/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="DK_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Denmark (subs + lines one-shot; single-query pattern)...")
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via 6-layer multi-DSO resolver",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Denmark v4.23 federation merger")
