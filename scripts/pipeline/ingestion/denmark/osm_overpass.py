"""
SSI Pipeline — Denmark L1 connector: OSM Overpass API.

Source ID:   DK-C1-osm-overpass
Publisher:   OpenStreetMap contributors via Overpass API
Licence:     ODbL (Open Database License) — attribution required

Feature classes ingested:
  - power=substation      expected 2000-3500 elements (way)
                          expected 500-1000 elements (node)
                          expected 10-30 elements (relation)
  - power=line             expected 3000-8000 ways (moderate density)
  - power=cable            urban underground (Copenhagen)
  - power=minor_line       MV distribution branches

DIFFERENCE FROM NEW ZEALAND CONNECTOR (Wave 3 P27 multi-DSO precedent):
  - LARGER SCALE MULTI-DSO NORDIC OFFSHORE-WIND WAVE 3 EVENT —
    Denmark is ~43k km² (~0.16× NZ at 268k km²) but 2× denser
    population + 4 major DSOs + Nordic OSM contributor density
    mature since 2005. Single-query per element type expected
    reliable but risk of 504 timeout for way-query at Nordic
    scale. Convention #56 partial-fetch preservation ready.

  - 6-LAYER MULTI-DSO RESOLVER + OFFSHORE-WIND CARVE-OUT — 4
    major DSOs (Radius Elnet Copenhagen + Cerius Zealand + N1
    Jutland + Trefor East Jutland+Fyn) + 3 minor + §4bis.5
    Copenhagen 6TH ENFORCEMENT. Simpler than NZ 29-EDB but more
    complex than Ireland/Korea single-DSO. Attribution reduces
    to 6-layer cascade:
      * Layer 1: Energinet TSO threshold (≥132 kV)
      * Layer 2: Banedanmark rail traction (25 kV AC)
      * Layer 3a: Offshore wind farm name match (Horns Rev + Anholt
                  + Kriegers Flak + Vesterhav + Rødsand + Nysted +
                  Middelgrunden)
      * Layer 3b: §4bis.5 Copenhagen metropolitan Radius geofence
                  (6TH COHORT-WIDE ENFORCEMENT)
      * Layer 4: Industrial captive (Novo Nordisk + Arla + Grundfos +
                 LEGO + Aalborg Portland + Danish Crown + Mærsk)
      * Layer 5: Region → dominant DSO map (5 regions × 4 major DSOs)
      * Layer 6: Energinet catch-all (safety net)

  - DANISH + ENGLISH + MINIMAL GERMAN SCHLESWIG per Convention #78
    BINDING 10th DECADE MILESTONE — Unicode NFC + case-insensitive
    lookup for:
      * Danish diacritics (æ ø å) via NFC normalization
      * Danish legal-form variants (A/S / ApS / AmbA / FmbA)
      * Predecessor rebrand cascades (DONG→Ørsted 2017 + SEAS-NVE→
        Cerius 2020 + Eltra+Elkraft→Energinet 2005 + TRE-FOR→Trefor
        2011+EWII 2016 + NRGi+Nordvest+Verdo+ELRO+Galten→N1 2019)
      * Offshore wind generation-vs-distribution separation
        (Ørsted/Vattenfall own offshore wind BUT grid=Energinet TSO)
    ~150-entry alias map (LARGEST cohort-wide alongside Switzerland
    150 + NZ 141 — multi-DSO + predecessor cascades drive entries).
    Preserves original tag in raw_attributes.osm_original_operator
    for audit trail. Estimated 40-120 alias-normalisation hits.

  - TSO THRESHOLD 132 kV — Energinet operates 400/150/132 kV
    backbone. Denmark unique 132 kV as regional transmission tier
    (Jutland historical). Below 132 kV → 4-DSO Layer 5 cascade.

Convention #56 visibly-honest degradation:
  - Missing OSM voltage tag → voltage_kv = None; owner defaults to
    Energinet catch-all (Layer 6 safety net; may be over-attributed
    but preserves visibly-honest state)
  - Missing operator tag → owner via 6-layer cascade
  - Overpass 429/504 → exponential backoff; graceful degradation

Convention #60 non-commercial provenance:
  - OSM (ODbL) only. Public open-data cross-validation deferred to
    Energinet Annual Report + Dansk Energi ENA public directory.

Discipline #36 cross-border filter — 5.0 km default tolerance (per
_base — existing DK entry from Mode 2 remediation for coastal storm
surge + offshore wind platforms + 4 HVDC interconnector precision;
Faroe Islands + Greenland pre-excluded via bounds.json — separate
country entries in ssi-index).

Discipline #41 line-substation pairing preserved.

Convention #78 §4bis.5 Layer 3 geofence — REQUIRED (6TH COHORT-WIDE
ENFORCEMENT at Copenhagen metropolitan Radius Elnet).
"""

from __future__ import annotations

import hashlib
import logging
import time
import urllib.request
import urllib.error
import urllib.parse
import json

from ._base import (
    SubstationRecord,
    TransmissionLineRecord,
    IngestionResult,
    apply_bounds_filter,
    assert_line_parity,
    emit_audit_sidecar,
    cache_path_for,
    now_utc_iso,
    resolve_owner_from_region_jurisdiction,
    normalise_owner_alias,
)

logger = logging.getLogger(__name__)

# ── Constants ────────────────────────────────────────────────────────────
SOURCE_ID = "DK-C1-osm-overpass"
OVERPASS_ENDPOINTS = [
    "https://overpass-api.de/api/interpreter",
    "https://overpass.kumi.systems/api/interpreter",
    "https://overpass.private.coffee/api/interpreter",
]

DEFAULT_TIMEOUT_SECS = 120
DEFAULT_QUERY_TIMEOUT_SECS = 90
USER_AGENT = "SSI-Index-Foundation/1.0 (+https://ikengassiindex.github.io)"

INITIAL_BACKOFF_SECS = 30
MAX_BACKOFF_SECS = 300
MAX_RETRY_ATTEMPTS = 4


# ── HTTP fetch with rate-limit backoff ──────────────────────────────────
def _fetch_overpass(query: str, timeout: int = DEFAULT_TIMEOUT_SECS) -> bytes:
    """Fetch Overpass query with retry + endpoint fallback."""
    cache = cache_path_for(query)
    if cache.exists():
        body = cache.read_bytes()
        logger.info("Cache hit: %s (%d bytes)", cache.name, len(body))
        return body

    data = urllib.parse.urlencode({"data": query}).encode("ascii")
    last_err: Exception | None = None

    for endpoint_idx, endpoint in enumerate(OVERPASS_ENDPOINTS):
        backoff = INITIAL_BACKOFF_SECS
        for attempt in range(MAX_RETRY_ATTEMPTS):
            try:
                req = urllib.request.Request(
                    endpoint,
                    data=data,
                    headers={
                        "User-Agent": USER_AGENT,
                        "Accept": "application/json",
                        "Content-Type": "application/x-www-form-urlencoded",
                    },
                )
                logger.info(
                    "Overpass POST %s (attempt %d/%d)",
                    endpoint, attempt + 1, MAX_RETRY_ATTEMPTS,
                )
                with urllib.request.urlopen(req, timeout=timeout) as r:
                    body = r.read()
                cache.write_bytes(body)
                return body
            except urllib.error.HTTPError as exc:
                last_err = exc
                if exc.code == 429:
                    logger.warning(
                        "Overpass 429 rate-limited; backing off %.0fs (attempt %d)",
                        backoff, attempt + 1,
                    )
                    time.sleep(backoff)
                    backoff = min(backoff * 2, MAX_BACKOFF_SECS)
                    continue
                elif exc.code in (502, 503, 504):
                    logger.warning(
                        "Overpass %d gateway; trying next endpoint after 5s",
                        exc.code,
                    )
                    time.sleep(5)
                    break
                elif exc.code == 406:
                    logger.warning("Overpass 406; retry in 5s")
                    time.sleep(5)
                    continue
                else:
                    logger.error("Overpass HTTP %d: %s", exc.code, exc.reason)
                    time.sleep(5)
                    continue
            except (urllib.error.URLError, TimeoutError) as exc:
                last_err = exc
                logger.warning("Overpass network error: %s; trying next endpoint after 5s", exc)
                time.sleep(5)
                break

    if last_err is not None:
        raise last_err
    raise RuntimeError("Overpass fetch failed on all endpoints without a specific error")


# ── Query builders ──────────────────────────────────────────────────────
_DK_AREA_HEADER = 'area["ISO3166-1"="DK"]->.dk'


def _query_substations(feature_type: str = "way") -> str:
    """Build Overpass query for one substation element type."""
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_DK_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.dk);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Denmark.

    Single-query safe for compact-scale country — Denmark ~43k km²
    with mature Nordic OSM contributor density (community established
    since 2005). Expected 3000-8000 lines within timeout budget."""
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_DK_AREA_HEADER};'
        f'(way["power"="line"](area.dk);'
        f'way["power"="cable"](area.dk);'
        f'way["power"="minor_line"](area.dk););'
        f'out geom tags;'
    )


# ── OSM element parsers ─────────────────────────────────────────────────
def _center_lat_lon(el: dict) -> tuple[float, float] | None:
    if el.get("type") == "node":
        lat, lon = el.get("lat"), el.get("lon")
    else:
        center = el.get("center") or {}
        lat, lon = center.get("lat"), center.get("lon")
    if lat is None or lon is None:
        return None
    return float(lat), float(lon)


def _parse_voltage_kv(tag_value: str | None) -> float | None:
    """Parse OSM voltage= tag (in volts, may be semicolon-separated)."""
    if not tag_value:
        return None
    values = []
    for token in tag_value.replace(",", "").split(";"):
        token = token.strip().rstrip("V").strip()
        try:
            v = float(token)
            if v > 0:
                values.append(v)
        except ValueError:
            continue
    if not values:
        return None
    max_v = max(values)
    return max_v / 1000.0 if max_v > 2000 else max_v


def _substation_from_osm(el: dict) -> SubstationRecord | None:
    """Parse OSM substation element.

    Denmark-specific: direct OSM operator= tag propagation with Danish
    + English + minimal German Schleswig alias normalization via
    Convention #78 BINDING 10th DECADE MILESTONE alias map (~150-entry
    Energinet TSO + 4 major DSOs + Banedanmark rail + Ørsted/Vattenfall
    offshore wind + predecessor rebrand cascades); if untagged, 6-layer
    multi-DSO resolver (Layer 1 Energinet TSO ≥132 kV OR Layer 2
    Banedanmark 25 kV AC traction OR Layer 3a offshore wind name-match
    OR Layer 3b §4bis.5 Copenhagen metropolitan Radius geofence (6TH
    COHORT-WIDE ENFORCEMENT) OR Layer 4 industrial captive OR Layer 5
    region → dominant DSO OR Layer 6 Energinet catch-all)."""
    coords = _center_lat_lon(el)
    if coords is None:
        return None
    lat, lon = coords
    tags = el.get("tags", {}) or {}

    osm_id = f"osm_{el.get('type', '?')}_{el.get('id', 0)}"
    voltage_kv = _parse_voltage_kv(tags.get("voltage"))
    name = tags.get("name") or tags.get("name:da") or tags.get("name:en")

    op_tag = (tags.get("operator") or "").strip()

    if op_tag:
        owner = normalise_owner_alias(op_tag)
        owner_provenance = (
            "osm_operator_tag_direct_alias_normalised" if owner != op_tag
            else "osm_operator_tag_direct"
        )
    else:
        # 6-layer multi-DSO resolver
        admin_code = (
            tags.get("addr:state")
            or tags.get("addr:region")
            or tags.get("is_in:region")
            or tags.get("region")
        )
        owner, owner_provenance = resolve_owner_from_region_jurisdiction(
            voltage_kv, lat, lon, admin_code=admin_code, name=name
        )

    return SubstationRecord(
        source_id=SOURCE_ID,
        feature_id=osm_id,
        latitude=lat,
        longitude=lon,
        voltage_kv=voltage_kv,
        owner=owner,
        operator_station_name=name,
        raw_attributes={
            "osm_type": el.get("type"),
            "osm_id": el.get("id"),
            "osm_tags": tags,
            "owner_provenance": owner_provenance,
            "osm_substation_subtype": tags.get("substation"),
            "osm_location": tags.get("location"),
            "osm_ref": tags.get("ref"),
            "osm_name_da": tags.get("name:da"),
            "osm_name_en": tags.get("name:en"),
            "osm_original_operator": op_tag if op_tag and op_tag != owner else None,
        },
    )


def _line_from_osm(el: dict) -> TransmissionLineRecord | None:
    """Parse OSM power=line element."""
    geom = el.get("geometry") or []
    if not geom or len(geom) < 2:
        return None
    coords: list[list[float]] = []
    for pt in geom:
        lat = pt.get("lat")
        lon = pt.get("lon")
        if lat is None or lon is None:
            continue
        coords.append([float(lon), float(lat)])
    if len(coords) < 2:
        return None

    tags = el.get("tags", {}) or {}
    voltage_kv = _parse_voltage_kv(tags.get("voltage"))
    op_tag = (tags.get("operator") or "").strip()
    owner = normalise_owner_alias(op_tag) if op_tag else None
    name = tags.get("name") or tags.get("name:da") or tags.get("name:en")

    return TransmissionLineRecord(
        source_id=SOURCE_ID,
        feature_id=f"osm_way_{el.get('id', 0)}",
        coordinates_multilinestring=[coords],
        voltage_kv=voltage_kv,
        owner=owner,
        line_name=name,
        raw_attributes={
            "osm_type": el.get("type"),
            "osm_id": el.get("id"),
            "osm_tags": tags,
            "osm_power_class": tags.get("power"),
            "osm_cables": tags.get("cables"),
            "osm_wires": tags.get("wires"),
            "osm_circuits": tags.get("circuits"),
        },
    )


# ── Public entry ─────────────────────────────────────────────────────────
def fetch(
    *,
    apply_bounds: bool = True,
    ingest_lines: bool = True,
) -> IngestionResult:
    """Fetch OSM Denmark substations + lines (single-query per element type).

    Compact-scale country — no partitioning needed. Uses OSM operator=
    tag with Danish + English + minimal German Schleswig alias
    normalisation per Convention #78 BINDING 10th DECADE MILESTONE,
    then 6-layer multi-DSO resolver (Energinet TSO OR Banedanmark rail
    OR offshore wind name-match OR §4bis.5 Copenhagen Radius geofence
    OR industrial captive OR region → DSO OR Energinet catch-all) for
    untagged. Convention #56 preserved."""
    result = IngestionResult(
        source_id=SOURCE_ID,
        fetched_at_utc=now_utc_iso(),
        source_url=OVERPASS_ENDPOINTS[0],
        provincial_scope="DK",
    )

    # ── Substations (split by element type) ──
    all_sub_bytes = bytearray()
    for feat_type in ("way", "node", "relation"):
        try:
            body = _fetch_overpass(_query_substations(feat_type))
        except Exception as exc:
            result.warnings.append(
                f"Convention #56 partial — OSM substation {feat_type} fetch failed: {exc}"
            )
            continue
        all_sub_bytes.extend(body)
        try:
            data = json.loads(body)
        except json.JSONDecodeError as exc:
            result.warnings.append(
                f"Convention #56 — OSM {feat_type} JSON parse error: {exc}"
            )
            continue
        for el in data.get("elements", []):
            rec = _substation_from_osm(el)
            if rec is not None:
                result.substations.append(rec)

    result.raw_bytes_fetched = len(all_sub_bytes)
    result.raw_sha256 = hashlib.sha256(bytes(all_sub_bytes)).hexdigest()
    logger.info("Parsed %d substations from OSM Overpass", len(result.substations))

    # ── Lines (single-query, no partitioning) ──
    if ingest_lines:
        try:
            body = _fetch_overpass(_query_lines())
            try:
                data = json.loads(body)
                for el in data.get("elements", []):
                    rec = _line_from_osm(el)
                    if rec is not None:
                        result.transmission_lines.append(rec)
            except json.JSONDecodeError as exc:
                result.warnings.append(
                    f"Convention #56 — OSM lines JSON parse error: {exc}"
                )
        except Exception as exc:
            result.warnings.append(
                f"Convention #56 partial — OSM lines fetch failed: {exc}"
            )
        logger.info("Parsed %d lines from OSM Overpass", len(result.transmission_lines))

    # ── Discipline #36 bounds filter (5.0 km coastline tolerance) ──
    if apply_bounds:
        subs_kept, subs_dropped = apply_bounds_filter(result.substations)
        lines_kept, lines_dropped = apply_bounds_filter(result.transmission_lines)
        result.substations = subs_kept
        result.transmission_lines = lines_kept
        if subs_dropped or lines_dropped:
            result.warnings.append(
                f"Discipline #36 — dropped {len(subs_dropped)} substations + "
                f"{len(lines_dropped)} lines outside Denmark polygon "
                "(5.0 km coastal + offshore wind territorial-waters "
                "+ 4 HVDC interconnector precision tolerance)."
            )

    # ── Owner enrichment summary ──
    if result.substations:
        from collections import Counter
        prov_counter = Counter(
            s.raw_attributes.get("owner_provenance", "none")
            for s in result.substations
        )
        direct = sum(
            v for k, v in prov_counter.items()
            if k.startswith("osm_operator_tag_direct")
        )
        energinet_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_1_Energinet_TSO")
            or k.startswith("region_jurisdiction_layer_6_Energinet_catch_all")
        )
        banedanmark = prov_counter.get(
            "region_jurisdiction_layer_2_Banedanmark_25kv_AC_traction", 0
        )
        offshore_wind = prov_counter.get(
            "region_jurisdiction_layer_3a_offshore_wind_name_match", 0
        )
        copenhagen_geofence = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_3_4bis5_6th_enforcement")
        )
        captive = prov_counter.get(
            "region_jurisdiction_layer_4_industrial_captive_name_match", 0
        )
        region_dso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_5_")
        )
        alias_normalised = prov_counter.get(
            "osm_operator_tag_direct_alias_normalised", 0
        )
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + "
            "%d Energinet_TSO + %d Banedanmark + %d offshore_wind + "
            "%d §4bis5_Copenhagen (6th enforcement) + %d captive + "
            "%d region_DSO",
            direct, alias_normalised, energinet_tso, banedanmark, offshore_wind,
            copenhagen_geofence, captive, region_dso,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} alias-normalised incl. Danish "
            f"(Energinet / Radius Elnet / Cerius / N1 / Trefor + æ ø å diacritics) "
            f"+ English (offshore wind Ørsted/Vattenfall generation NOT DSO — "
            f"routed to Energinet) + minimal German Schleswig + predecessor "
            f"rebrands (DONG→Ørsted 2017 + SEAS-NVE→Cerius 2020 + Eltra+Elkraft→"
            f"Energinet 2005 + TRE-FOR→Trefor 2011+EWII 2016 + Norlys 2019 "
            f"merger NRGi+Nordvest+Verdo+ELRO+Galten→N1) per Convention #78 "
            f"BINDING 10th DECADE MILESTONE); "
            f"{energinet_tso} Energinet_TSO + {banedanmark} Banedanmark + "
            f"{offshore_wind} offshore_wind + {copenhagen_geofence} "
            f"§4bis5_Copenhagen_6TH_ENFORCEMENT + {captive} industrial_captive + "
            f"{region_dso} region_DSO "
            f"({100 * (energinet_tso + banedanmark + offshore_wind + copenhagen_geofence + captive + region_dso) / len(result.substations):.1f}%)."
        )

    # ── Discipline #41 parity ──
    parity_ok, findings = assert_line_parity(result, outbound_border_ok=False)
    for f in findings:
        logger.info("Discipline #41: %s", f)

    return result


# ── CLI harness ──────────────────────────────────────────────────────────
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass DK L1 connector — 6-layer multi-DSO cascade (Energinet TSO OR Banedanmark rail OR offshore wind name-match OR §4bis.5 Copenhagen Radius geofence 6TH COHORT-WIDE ENFORCEMENT OR industrial captive OR region → dominant DSO OR Energinet catch-all) + Convention #78 BINDING 10th DECADE MILESTONE (Danish + English + minimal German Schleswig + Danish diacritics æ ø å + predecessor rebrand cascades DONG→Ørsted 2017 + SEAS-NVE→Cerius 2020 + Eltra+Elkraft→Energinet 2005 + TRE-FOR→Trefor 2011+EWII 2016 + Norlys 2019 merger) — FIRST Nordic offshore wind Wave 3 event")
    parser.add_argument("--skip-lines", action="store_true",
                        help="Skip line ingestion (subs only)")
    parser.add_argument("--no-bounds", action="store_true",
                        help="Skip Discipline #36 filter")
    parser.add_argument("--verbose", "-v", action="store_true")
    args = parser.parse_args()

    logging.basicConfig(
        level=logging.DEBUG if args.verbose else logging.INFO,
        format="%(asctime)s %(levelname)s %(name)s: %(message)s",
    )

    result = fetch(
        apply_bounds=not args.no_bounds,
        ingest_lines=not args.skip_lines,
    )

    parity_ok, findings = assert_line_parity(result, outbound_border_ok=False)
    audit_path = emit_audit_sidecar(result, parity_findings=findings)

    print(f"\n{SOURCE_ID} fetch complete")
    print(f"  substations:        {len(result.substations):,}")
    print(f"  transmission_lines: {len(result.transmission_lines):,}")
    print(f"  raw_bytes_fetched:  {result.raw_bytes_fetched:,}")
    print(f"  raw_sha256:         {result.raw_sha256}")
    print(f"  audit_sidecar:      {audit_path}")

    from collections import Counter
    prov = Counter(
        s.raw_attributes.get("owner_provenance", "none")
        for s in result.substations
    )
    print("  Owner-provenance distribution:")
    for k, v in prov.most_common():
        print(f"    {v:>5}  {k}")

    ops = Counter(s.owner for s in result.substations if s.owner)
    print(f"  Distinct operators: {len(ops)}")
    print("  Top 15 operators:")
    for k, v in ops.most_common(15):
        print(f"    {v:>5}  {k}")

    for w in result.warnings:
        print(f"  ⚠ {w}")


if __name__ == "__main__":
    _cli_main()
