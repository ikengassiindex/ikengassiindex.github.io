"""
SSI Pipeline — Slovenia v4.23 federation merger.

Merges OSM Overpass L1 ingestion into slovenia/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with DSO alias normalisation: legal-form
     dd/doo strip + historical "Elektro-Slovenija" → "ELES d.o.o.") OR
     region-jurisdiction fallback per _base.py::resolve_owner_from_region_jurisdiction
     (5-DSO NUTS-3 map + ELES TSO threshold ≥110 kV).
     Slovenia baseline: 0% owner tagged. Expected post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 100% at 66 kV — LARGE
     multi-tier fill opportunity from OSM: EHV 400/220 kV + HV 110 kV +
     MV 20/35 kV).
  ...
  5. Line densification: append + dedupe by midpoint 100m + voltage ±20%.
     Note: baseline line ratio 27.9 (task #182 outlier — MV deep-ingestion);
     expected line densification may compound this.
...
SSI_DATA_JSON = REPO_ROOT / "slovenia" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "slovenia" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "slovenia" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "SI_v43_") -> str:
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            v43_prov[SOURCE_ID] = {
                ...
                "nuts3_code_detected": ov.raw_attributes.get("nuts3_code_detected"),
            }
            ...
            new_sub = {
                ...
                "osm_feature_id": ov.feature_id,
                "osm_substation_subtype": ov.raw_attributes.get("osm_substation_subtype"),
                "region": ov.raw_attributes.get("nuts3_code_detected"),  # NUTS-3 SI### if OSM tagged
                "province": None,
                "version": "4.2",
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (slovenia/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="SI_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Slovenia (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Slovenia v4.23 federation merger")
