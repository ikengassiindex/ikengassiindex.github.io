"""
SSI Pipeline — Korea v4.23 federation merger.

Merges OSM Overpass L1 ingestion into korea/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Hangul + Latin RRK/M-R + English
     acronym cohabitation + Chaebol captive + KEPCO GENCOs + Hangul
     legal-form (주식회사/㈜) + KECO pre-1982 predecessor Unicode NFC
     alias normalisation per Convention #78 BINDING 8th enforcement)
     OR 4-layer KEPCO monopoly fallback per _base.py::
     resolve_owner_from_region_jurisdiction (Layer 1 KHNP nuclear OR
     Layer 2 industrial captive OR Layer 3 admin default OR Layer 4
     KEPCO catch-all). Korea baseline: 87.0% owner tagged (LARGEST
     cohort-wide baseline attribution — Ireland 0.0% + Switzerland
     0.0% + Iceland 0.0% + Greek 0.0% + prior Wave 2 baselines).
     Expected post-merge: ~100% (SIMPLEST cohort-wide via KEPCO
     monopoly).

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 88% populated so
     moderate fills expected — 152 subs have null voltage).
  ...
  4b. Step 4b — Retroactive owner attribution on baseline subs without
      ...
      4-layer KEPCO monopoly resolver to close 100% owner coverage.
      Expected LOWEST cohort-wide retroactive diversity (1 owner class
      — KEPCO catch-all handles ~99% of unmatched; nuclear/captive
      Layer 1-2 already handled at initial merge).
...
OSM tag (including Hangul native forms 한국전력공사/한전, Latin
transliterations (RRK Hanguk Jeollyeok Gongsa + M-R Han'guk Chŏllyŏk
Kongsa), English acronyms (KEPCO/KHNP/KPX/KDN), Chaebol captives
(POSCO/Samsung/SK Hynix/Hyundai/LG Chem), Hangul legal-form suffixes
(주식회사/㈜), and KECO pre-1982 predecessor legacy) in
...
SSI_DATA_JSON = REPO_ROOT / "korea" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "korea" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "korea" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "KR_v43_") -> str:
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            v43_prov[SOURCE_ID] = {
                ...
                "osm_name": osm_name,
                "osm_name_ko": ov.raw_attributes.get("osm_name_ko"),
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply 4-layer KEPCO monopoly resolver to baseline subs without OSM match.
    ...
    Korean simplification: KEPCO monopoly makes retroactive attribution
    trivial — Layer 1 KHNP nuclear name-match handled at initial merge;
    Layer 2 industrial captive name-match handled at initial merge;
    Layer 3 admin default empirically dormant; Layer 4 KEPCO catch-all
    covers the remaining ~99% of unmatched baseline subs. Every
    unmatched baseline sub gets attributed (100% coverage post-4b).
    Analogous to Greek P22 DEDDIE + Costa Rica P13 ICE + Ireland P25
    ESB Networks simplifications. LOWEST cohort-wide retroactive
    diversity expected (1 owner class — KEPCO — dominant).
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        # Korean admin (do/si) for API-compat (Korean resolver may honor for
        # forward-compat; empirically ~0 hits — all route to KEPCO catch-all)
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "(500m proximity miss). Same 4-layer KEPCO monopoly resolver "
                    "as OSM path (Layer 1 KHNP nuclear name OR Layer 2 industrial "
                    "captive name OR Layer 3 admin default OR Layer 4 KEPCO "
                    "catch-all). Codified in Costa Rica Priority 13 closure; "
                    "Korean simplification via KEPCO monopoly makes retroactive "
                    "attribution trivial (Greek DEDDIE + Costa Rica ICE + Ireland "
                    "ESB Networks precedent)."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (korea/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="KR_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Korea (subs + lines one-shot; single-query pattern)...")
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via 4-layer KEPCO monopoly resolver",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Korea v4.23 federation merger")
