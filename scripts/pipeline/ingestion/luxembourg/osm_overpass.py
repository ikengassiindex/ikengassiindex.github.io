"""
SSI Pipeline — Luxembourg L1 connector: OSM Overpass API.

Source ID:   BE-C1-osm-overpass
...
Feature classes ingested:
  - power=substation      expected 2k-4k elements
  - power=line             expected 5k-15k ways
  - power=cable            urban underground
  - power=minor_line       LV distribution branches

DIFFERENCE FROM BELGIUM CONNECTOR:
  - COMPACT SCALE — Luxembourg is a tiny 2.6k km² (~1/250 Australia). Single-query
    per element type is safe; no bbox partitioning needed for lines.

  - REGION-JURISDICTION OWNER FALLBACK — 3-region federal (Flanders/
    Wallonia/Brussels-Capital) × distinct DNSPs. Flanders → Fluvius
    (post-2018 Eandis+Infrax merger, single DNSP). Wallonia → ORES
    (default) + Resa (Liège metro geofence). Brussels-Capital → Sibelga.
    All ≥150 kV → Elia (TSO).

  - DNSP ALIAS NORMALISATION — historical OSM tags may show pre-merger
    names (Eandis/Infrax → Fluvius; Tecteo → Resa). Merge script
    normalises via _base.py::normalise_owner_alias.
...
Convention #60 non-commercial provenance:
  - OSM (ODbL) only. Elia open-data cross-validation deferred.
...
DEFAULT_TIMEOUT_SECS = 90  # small country — shorter timeout OK
...
# ── Query builders ──────────────────────────────────────────────────────
_LU_AREA_HEADER = 'area["ISO3166-1"="LU"]->.lu'


def _query_substations(feature_type: str = "way") -> str:
    """Build Overpass query for one substation element type.

    Small-country: single query per element type is fine (Austria pattern).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_LU_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.lu);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Belgium.

    Single-query safe for small country — Luxembourg bbox ~2.6k km² (SMALLEST cohort-wide).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_LU_AREA_HEADER};'
        f'(way["power"="line"](area.lu);'
        f'way["power"="cable"](area.lu);'
        f'way["power"="minor_line"](area.lu););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Belgium-specific: direct OSM operator= tag propagation with DNSP alias
    normalisation; if untagged, region-jurisdiction fallback.
    """
    ...
    osm_region = tags.get("addr:state") or tags.get("region") or tags.get("addr:region")
    ...
    if op_tag:
        ...
        owner, owner_provenance = resolve_owner_from_monopoly_default(lat, lon, voltage_kv)
...
def fetch(
    *,
    ...
    """Fetch OSM Luxembourg substations + lines (single-query per element type).

    Small country — no partitioning needed. Uses OSM operator= tag with
    DNSP alias normalisation, then region-jurisdiction fallback for
    ...
    # ── Substations (split by element type — Austria pattern) ──
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Luxembourg polygon "
    ...
    if result.substations:
        ...
        monopoly_default = sum(
            v for k, v in prov_counter.items() if k.startswith("monopoly_fallback")
        )
        unresolved = sum(
            v for k, v in prov_counter.items() if k.startswith("unresolved")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d monopoly_default + %d unresolved",
            direct, alias_normalised, monopoly_default, unresolved,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} DNSP-alias-normalised e.g. Eandis→Fluvius); "
            f"{monopoly_default} monopoly_default_fallback "
            f"({100 * monopoly_default / len(result.substations):.1f}%); "
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass LU L1 connector")
