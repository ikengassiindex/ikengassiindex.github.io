"""
SSI Pipeline — Luxembourg v4.23 federation merger.

Merges OSM Overpass L1 ingestion into belgium/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with DNSP alias normalisation:
     Eandis/Infrax→Fluvius, Tecteo→Resa) OR region-jurisdiction fallback
     per _base.py::resolve_owner_from_region_jurisdiction.
     Luxembourg baseline: 0% owner tagged. Expected post-merge: ~100% (monopoly pattern).

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 606/1219 = 49.7% null →
     LARGE FILL OPPORTUNITY from OSM tags).
  ...
  6. Audit trail with owner_provenance + DNSP-alias normalisation flag.
...
SSI_DATA_JSON = REPO_ROOT / "luxembourg" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "luxembourg" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "luxembourg" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "LU_v43_") -> str:
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            new_sub = {
                ...
                "osm_feature_id": ov.feature_id,
                "osm_substation_subtype": ov.raw_attributes.get("osm_substation_subtype"),
                "region": None,   # province — L2 spatial join fills at rescore
                "province": None,
                "version": "4.2",
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (luxembourg/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="LU_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Luxembourg (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Luxembourg v4.23 federation merger")
