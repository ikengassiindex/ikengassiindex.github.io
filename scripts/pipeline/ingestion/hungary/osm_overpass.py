"""
SSI Pipeline — Hungary L1 connector: OSM Overpass API.
...
DIFFERENCE FROM BELGIUM/NL CONNECTOR:
  - COMPACT SCALE — Hungary is ~93k km² (~3× Belgium) (~1/250 Australia). Single-query
...
# ── Query builders ──────────────────────────────────────────────────────
_HU_AREA_HEADER = 'area["ISO3166-1"="HU"]->.hu'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_HU_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.hu);'
        f'out center tags;'
    )
...
def _query_lines() -> str:
    ...
    Single-query safe for small country — Hungary bbox ~93k km².
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_HU_AREA_HEADER};'
        f'(way["power"="line"](area.hu);'
        f'way["power"="cable"](area.hu);'
        f'way["power"="minor_line"](area.hu););'
        f'out geom tags;'
    )
...
def fetch(
    *,
    ...
    """Fetch OSM Hungary substations + lines (single-query per element type).
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Hungary polygon "
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass HU L1 connector")
