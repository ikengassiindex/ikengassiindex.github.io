"""
SSI Pipeline — Switzerland v4.23 federation merger.

Merges OSM Overpass L1 ingestion into switzerland/ssi-data.json +
grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with 4-language script cohabitation
     (German + French + Italian + Romansh) + trilingual legal-form
     (AG/SA/SpA) + Swiss diacritics + NOK/Bernische Kraftwerke/EEF/EOS/
     Rätia Energie predecessor Unicode alias normalisation per
     Convention #78 BINDING 6th enforcement) OR voltage-class × Layer
     3 8-way multi-metro/multi-cantonal DSO fallback per
     _base.py::resolve_owner_from_region_jurisdiction (Swissgrid
     ≥220 kV OR EWZ/SIG/SIL/IWB/AET/Repower/Groupe E/Romande Energie/
     CKW/BKW via geofence OR Axpo Grid default). Switzerland
     baseline: 0.0% owner tagged (empirical audit — clean slate
     cohort-wide signature).
     Expected post-merge: ~100% (LAYER 3 8-WAY multi-metro/multi-
     cantonal cohort-wide).

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 99.9% populated so few
     fills expected).
  ...
  4b. Step 4b — Retroactive owner attribution on baseline subs without
      ...
      resolver to close 100% owner coverage.

  5. Line densification: append + dedupe by midpoint 100m + voltage ±20%.
     Major line densification expected (baseline 625 lines is
     significantly under Central European transmission density;
     expected 4-10× line growth to ~2,500-6,000).
...
OSM tag (including 4-language Swiss diacritics — German ä ö ü ß +
French à é è ê ï ô ù û + Italian à è ì ò ù + Romansh — trilingual
AG/SA/SpA legal-form variants, and NOK/Bernische Kraftwerke/EEF/EOS/
Rätia Energie predecessor legacy) in raw_attributes.osm_original_operator
for audit trail.
"""
...
SSI_DATA_JSON = REPO_ROOT / "switzerland" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "switzerland" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "switzerland" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "CH_v43_") -> str:
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply voltage-class × Layer 3 8-way multi-metro/multi-cantonal DSO
    resolver to baseline subs without OSM match.
    ...
    Switzerland multi-DSO + multi-metro complexity: 8-way Layer 3
    geofence composition means retroactive attribution surfaces per-DSO
    breakdown by territorial partition (EWZ Zürich metro + SIG Geneva
    metro + SIL Lausanne metro + IWB Basel-Stadt metro + AET Ticino +
    Repower Graubünden + Groupe E Fribourg/Neuchâtel + Romande Energie
    Vaud/Valais + CKW Zentralschweiz + BKW Bern/Jura + Axpo Grid
    Deutschschweiz default) — RICHEST cohort-wide (surpasses Iceland
    5-way + Poland 4-way + Czechia 3-way). Every unmatched baseline
    sub gets attributed (100% coverage post-4b).
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        # NUTS-3 for API-compat (Switzerland resolver empirically expects None —
        # Wave 2/3 OSM density hypothesis — but forward-compat surface preserved)
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "(500m proximity miss). Same voltage-class × Layer 3 8-way "
                    "multi-metro/multi-cantonal DSO resolver as OSM path (Swissgrid "
                    "≥220 kV OR EWZ/SIG/SIL/IWB metro OR AET/Repower/Groupe E/"
                    "Romande Energie/CKW/BKW cantonal OR Axpo Grid default). "
                    "Codified in Costa Rica Priority 13 closure; Switzerland "
                    "multi-DSO + multi-metro complexity extends the pattern with "
                    "8-way territorial partition breakdown (RICHEST cohort-wide "
                    "post-Iceland 5-way + Poland 4-way + Czechia 3-way)."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (switzerland/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="CH_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Switzerland (subs + lines one-shot; single-query pattern)...")
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via voltage-class × Layer 3 8-way multi-metro/multi-cantonal DSO resolver",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Switzerland v4.23 federation merger")
