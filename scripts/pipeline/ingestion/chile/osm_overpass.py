"""
SSI Pipeline — Chile L1 connector: OSM Overpass API.
...
DIFFERENCE FROM BELGIUM/NETHERLANDS CONNECTOR:
  - COMPACT SCALE — Chile is a narrow 4,400 km N-S strip (~1/250 Australia). Single-query
...
# ── Query builders ──────────────────────────────────────────────────────
_CL_AREA_HEADER = 'area["ISO3166-1"="CL"]->.cl'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_CL_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.cl);'
        f'out center tags;'
    )
...
def _query_lines() -> str:
    ...
    Single-query safe for small country — Chile bbox — narrow N-S strip ~750k km².
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_CL_AREA_HEADER};'
        f'(way["power"="line"](area.cl);'
        f'way["power"="cable"](area.cl);'
        f'way["power"="minor_line"](area.cl););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    if op_tag:
        ...
        owner, owner_provenance = resolve_owner_from_region_jurisdiction(
            lat, lon, voltage_kv, osm_region=osm_region
        )
...
def fetch(
    *,
    ...
    """Fetch OSM Chile substations + lines (single-query per element type).
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Chile polygon "
    ...
    if result.substations:
        ...
        region_fallback = sum(
            v for k, v in prov_counter.items() if k.startswith("region_fallback")
        )
        unresolved = sum(
            v for k, v in prov_counter.items() if k.startswith("region_unresolved")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d region_fallback + %d unresolved",
            direct, alias_normalised, region_fallback, unresolved,
        )
        result.warnings.append(
            ...
            f"{region_fallback} region_jurisdiction_fallback "
            f"({100 * region_fallback / len(result.substations):.1f}%); "
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass CL L1 connector")
