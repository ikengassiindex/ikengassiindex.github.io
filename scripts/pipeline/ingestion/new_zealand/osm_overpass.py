"""
SSI Pipeline — New Zealand L1 connector: OSM Overpass API.

Source ID:   NZ-C1-osm-overpass
...
Feature classes ingested:
  - power=substation      expected 1500-2500 elements (way)
                          expected 500-1000 elements (node)
                          expected 5-20 elements (relation)
  - power=line             expected 4000-10000 ways (moderate density)
  - power=cable            urban underground (Auckland + Wellington)
  - power=minor_line       MV distribution branches

DIFFERENCE FROM KOREA CONNECTOR (Wave 3 P26 KEPCO monopoly precedent):
  - LARGEST SCALE MULTI-DSO WAVE 3 EVENT — NZ is ~268k km²
    (~2.7× Korea at 100k km²) with mature Anglo OSM contributor
    density (NZ OSM community established since 2005). Single-query
    per element type expected reliable. 504 gateway timeouts
    possible on line query given elevated line-substation ratio
    projection — Convention #56 partial-fetch preservation ready.

  - RICHEST MULTI-DSO ARCHITECTURE cohort-wide — 29 EDBs across
    16 regions with §4bis.5 Layer 3 5th cohort-wide enforcement at
    Auckland metropolitan (Vector vs Counties Energy split). NOT
    Korean-style monopoly (KEPCO) nor Irish-style single-DSO (ESB
    Networks) — RICHER than Swiss 8-way (previous cohort-wide max).
    Attribution reduces to 7-layer cascade:
      * Layer 1: Transpower TSO threshold (≥110 kV)
      * Layer 2: KiwiRail rail traction (25 kV AC)
      ...
      * Layer 5b: Canterbury soft-boundary (Orion + MainPower +
                  Alpine + Waitaki)
      * Layer 6: Region → dominant EDB map (14 other regions)
      * Layer 7: Transpower catch-all (safety net)

  - ENGLISH DOMINANT + MĀORI TE REO DIACRITIC COHABITATION per
    Convention #78 BINDING 9th enforcement — Unicode NFC + case-
    insensitive lookup for:
      * English legal-form variants (Ltd / Limited / PLC — rare)
      * Māori Te Reo native names (Aotearoa cohabit + Ara Whanui
        KiwiRail)
      * Māori diacritics (macrons ā ē ī ō ū) via NFC normalization
      * Generation-retail separation (Meridian/Mercury/Genesis/
        Contact/Trustpower own generation but grid is Transpower)
      * Historical predecessors (ECNZ 1987-1996 + NZED 1946-1987 +
        AEPB pre-1993 + council electricity departments)
    ~120-entry alias map (LARGEST cohort-wide alongside Switzerland
    150 — 29 EDBs drive most entries). Preserves original tag in
    raw_attributes.osm_original_operator for audit trail. Estimated
    30-80 alias-normalisation hits (LOWER than Korean 198 given
    English dominance — cumulative 9-country post-NZ: ~20,880-20,930
    — 2,088-2,093× above BINDING threshold).

  - TSO THRESHOLD 110 kV — Transpower operates 220/110/66 kV
    backbone. NZ 110 kV is MAIN transmission tier (like Ireland
    EirGrid). Below 110 kV → 29-EDB Layer 3 §4bis.5 + admin cascade.

Convention #56 visibly-honest degradation:
  - Missing OSM voltage tag → voltage_kv = None; owner defaults to
    Transpower catch-all (Layer 7 safety net; may be over-attributed
    but preserves visibly-honest state)
  - Missing operator tag → owner via 7-layer cascade
...
Convention #60 non-commercial provenance:
  - OSM (ODbL) only. Public open-data cross-validation deferred to
    Transpower Annual Report + Electricity Authority public registry.

Discipline #36 cross-border filter — 5.0 km default tolerance (per
_base — existing NZ entry from Mode 2 remediation for Pacific
coastline + Fiordland + Stewart Island + territorial extensions
Chatham + Kermadec + Tokelau via bounds.json 24-feature polygon).

Discipline #41 line-substation pairing preserved.

Convention #78 §4bis.5 Layer 3 geofence — REQUIRED (5TH COHORT-WIDE
ENFORCEMENT at Auckland metropolitan Vector vs Counties Energy split).
"""
...
# ── Query builders ──────────────────────────────────────────────────────
_NZ_AREA_HEADER = 'area["ISO3166-1"="NZ"]->.nz'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_NZ_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.nz);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within New Zealand.

    Single-query safe for moderate-scale country — NZ ~268k km² with
    mature Anglo OSM contributor density (community established since
    2005). Expected 4000-10000 lines within timeout budget."""
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_NZ_AREA_HEADER};'
        f'(way["power"="line"](area.nz);'
        f'way["power"="cable"](area.nz);'
        f'way["power"="minor_line"](area.nz););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    NZ-specific: direct OSM operator= tag propagation with English
    dominant + Māori Te Reo diacritic cohabitation via Convention #78
    BINDING 9th enforcement alias map (120-entry Transpower/29-EDB/
    KiwiRail/NZAS/industrial captive/generation-retail separation
    normalization); if untagged, 7-layer multi-DSO resolver (Layer 1
    Transpower TSO ≥110 kV OR Layer 2 KiwiRail 25 kV AC traction OR
    Layer 3 NZAS Tiwai Point OR Layer 4 industrial captive OR
    Layer 5a §4bis.5 Auckland Vector/Counties Energy geofence
    (5TH COHORT-WIDE ENFORCEMENT) OR Layer 5b Canterbury soft-boundary
    OR Layer 6 region → dominant EDB OR Layer 7 Transpower catch-all)."""
    ...
    name = tags.get("name") or tags.get("name:mi") or tags.get("name:en")
...
def fetch(
    *,
    ...
    """Fetch OSM NZ substations + lines (single-query per element type).

    Moderate-scale country — no partitioning needed. Uses OSM operator=
    tag with English + Māori Te Reo alias normalisation per Convention
    #78 BINDING 9th enforcement, then 7-layer multi-DSO resolver
    (Transpower TSO OR KiwiRail traction OR NZAS captive OR industrial
    captive OR §4bis.5 Auckland Vector/Counties Energy geofence OR
    Canterbury soft-boundary OR region → EDB OR Transpower catch-all)
    for untagged. Convention #56 preserved."""
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside NZ polygon "
                "(5.0 km Pacific coastline + Fiordland + Stewart Island "
                "+ Chatham/Kermadec/Tokelau territorial extensions)."
            )
    ...
    if result.substations:
        ...
        transpower_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_1_Transpower_TSO")
            or k.startswith("region_jurisdiction_layer_7_Transpower_catch_all")
        )
        kiwirail = prov_counter.get(
            "region_jurisdiction_layer_2_KiwiRail_25kv_AC_traction", 0
        )
        nzas = prov_counter.get(
            "region_jurisdiction_layer_3_NZAS_Tiwai_Point_name_match", 0
        )
        captive = prov_counter.get(
            "region_jurisdiction_layer_4_industrial_captive_name_match", 0
        )
        auckland_geofence = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_3_4bis5_5th_enforcement")
        )
        canterbury_soft = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_5b_Canterbury")
        )
        region_dso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_layer_6_")
        )
        ...
        logger.info(
            ...
            "%d Transpower_TSO + %d KiwiRail + %d NZAS + %d captive + "
            "%d §4bis5_Auckland (5th enforcement) + %d Canterbury_soft + "
            "%d region_DSO",
            direct, alias_normalised, transpower_tso, kiwirail, nzas,
            captive, auckland_geofence, canterbury_soft, region_dso,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} alias-normalised incl. English "
            f"(Transpower / Vector / Orion NZ / Wellington Electricity / 29 EDBs) "
            f"+ Māori Te Reo (Aotearoa cohabit + Ara Whanui KiwiRail) "
            f"+ generation-retail separation (Meridian/Mercury/Genesis/Contact "
            f"NOT DSO — routed to Transpower) + ECNZ pre-1996 unbundling "
            f"predecessor per Convention #78 BINDING 9th enforcement); "
            f"{transpower_tso} Transpower_TSO + {kiwirail} KiwiRail + "
            f"{nzas} NZAS + {captive} industrial_captive + "
            f"{auckland_geofence} §4bis5_Auckland_5TH_ENFORCEMENT + "
            f"{canterbury_soft} Canterbury_soft + {region_dso} region_DSO "
            f"({100 * (transpower_tso + kiwirail + nzas + captive + auckland_geofence + canterbury_soft + region_dso) / len(result.substations):.1f}%)."
        )
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass NZ L1 connector — RICHEST multi-DSO 7-layer cascade (Transpower TSO OR KiwiRail traction OR NZAS captive OR industrial captive OR §4bis.5 Auckland Vector/Counties Energy geofence 5TH COHORT-WIDE ENFORCEMENT OR Canterbury soft-boundary OR region → dominant EDB OR Transpower catch-all) + Convention #78 BINDING 9th-enforcement test (English dominant + Māori Te Reo Aotearoa/Ara Whanui + generation-retail separation Meridian/Mercury/Genesis/Contact NOT DSO + ECNZ 1996 unbundling predecessor) — FIRST Southern Hemisphere Wave 3 event")
