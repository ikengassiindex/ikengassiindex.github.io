"""
SSI Pipeline — New Zealand v4.23 federation merger.

Merges OSM Overpass L1 ingestion into new-zealand/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with English dominant + Māori Te Reo
     diacritic cohabitation + generation-retail separation Meridian/
     Mercury/Genesis/Contact NOT DSO + ECNZ 1996 unbundling predecessor
     Unicode NFC alias normalisation per Convention #78 BINDING 9th
     enforcement) OR 7-layer multi-DSO resolver per _base.py::
     resolve_owner_from_region_jurisdiction (Layer 1 Transpower TSO
     ≥110 kV OR Layer 2 KiwiRail 25 kV AC OR Layer 3 NZAS Tiwai Point
     OR Layer 4 industrial captive OR Layer 5a §4bis.5 Auckland Vector/
     Counties Energy geofence 5TH COHORT-WIDE ENFORCEMENT OR Layer 5b
     Canterbury soft-boundary OR Layer 6 region → dominant EDB OR
     Layer 7 Transpower catch-all). NZ baseline: 0.0% owner tagged
     (CLEAN SLATE PATTERN — Ireland-analog).
     Expected post-merge: ~100% (RICHEST cohort-wide via 29-EDB multi-DSO).
  ...
  3. Voltage fill on matched pairs (baseline 99%+ populated so few
     fills expected — 220/110/66/33/22/11 kV tiers).
  ...
  4b. Step 4b — Retroactive owner attribution on baseline subs without
      ...
      7-layer multi-DSO resolver to close 100% owner coverage.
      Expected RICHEST cohort-wide retroactive diversity (25-35 owner
      classes given 29 EDBs + Layer 1-4 industrial + §4bis.5 Auckland
      + Canterbury soft-boundary).
...
OSM tag (including English legal-forms Ltd/Limited/PLC, Māori Te Reo
diacritic macrons ā ē ī ō ū via NFC composition, Aotearoa cohabit +
Ara Whanui KiwiRail Te Reo names, generation-retail separation
Meridian/Mercury/Genesis/Contact/Trustpower flag-routing, ECNZ 1987-
1996 pre-unbundling predecessor, NZED 1946-1987 pre-corporatisation
predecessor) in raw_attributes.osm_original_operator for audit trail.
"""
...
SSI_DATA_JSON = REPO_ROOT / "new-zealand" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "new-zealand" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "new-zealand" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "NZ_v43_") -> str:
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            v43_prov[SOURCE_ID] = {
                ...
                "osm_name": osm_name,
                "osm_name_mi": ov.raw_attributes.get("osm_name_mi"),
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply 7-layer multi-DSO resolver to baseline subs without OSM match.
    ...
    NZ RICHEST cohort-wide multi-DSO retroactive attribution:
    - Layer 1 Transpower TSO ≥110 kV (rare — most baseline subs are
      distribution tier 33/22/11 kV)
    - Layer 2 KiwiRail 25 kV AC traction (name-match)
    - Layer 3 NZAS Tiwai Point Southland (name-match)
    - Layer 4 Industrial captive Methanex/Fonterra (name-match)
    - Layer 5a §4bis.5 Auckland Vector vs Counties Energy geofence
      (5TH COHORT-WIDE ENFORCEMENT — DOMINANT retroactive for
      Auckland region)
    - Layer 5b Canterbury soft-boundary (Orion + MainPower + Alpine +
      Waitaki — DOMINANT retroactive for Canterbury region)
    - Layer 6 Region → dominant EDB (DOMINANT retroactive for
      14 other regions)
    - Layer 7 Transpower catch-all (safety net for edge cases)

    Every unmatched baseline sub gets attributed (100% coverage post-4b).
    Analogous to Swiss 8-way multi-DSO complexity but RICHER (29 EDBs).
    HIGHEST cohort-wide retroactive diversity expected (25-35 distinct
    owner classes).
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        # NZ 16 regions admin (populated from baseline enrichment)
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "(500m proximity miss). Same 7-layer multi-DSO resolver as OSM "
                    "path (Layer 1 Transpower TSO ≥110 kV OR Layer 2 KiwiRail 25 kV "
                    "AC traction OR Layer 3 NZAS Tiwai Point OR Layer 4 industrial "
                    "captive OR Layer 5a §4bis.5 Auckland Vector/Counties Energy "
                    "geofence 5TH COHORT-WIDE ENFORCEMENT OR Layer 5b Canterbury "
                    "soft-boundary OR Layer 6 region → dominant EDB OR Layer 7 "
                    "Transpower catch-all). Codified in Costa Rica Priority 13 "
                    "closure; NZ RICHEST cohort-wide via 29-EDB multi-DSO "
                    "architecture (Swiss 8-way precedent SUPERSEDED)."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (new-zealand/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="NZ_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM NZ (subs + lines one-shot; single-query pattern)...")
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via 7-layer multi-DSO resolver",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="New Zealand v4.23 federation merger")
