"""SSI Pipeline — Slovakia v4.23 ingestion package.
...
connector authoring time:
  ...
  - SEPS (Slovenska elektrizacna prenosova sustava a.s.): state TSO
    ...
    110 kV transmission
  - ZSD (Zapadoslovenska distribucna a.s.): West Slovakia DSO,
    NUTS-3: SK010 + SK021 + SK022 + SK023 (parent: ZSE 51 state / 49 E.ON)
  - SSD (Stredoslovenska distribucna a.s.): Centre Slovakia DSO,
    NUTS-3: SK031 + SK032 (parent: SSE 51 state / 49 EPH)
  - VSD (Vychodoslovenska distribucna a.s.): East Slovakia DSO,
  ...
  - Slovak Unicode NFC alias normalisation (c s z d l n t l r a i o u y)
  - Cyrillic alias handling (Rusyn + Ukrainian minority OSM in eastern
    Slovakia - Presov + Kosice bordering Ukraine)
    SEPS/ZSD/SSD/VSD Cyrillic renderings preemptively mapped per
    Convention #78 BINDING
  - Slovak typographic-quote variants (like Czech/German/Latvian)
    preemptively mapped per Convention #78 BINDING (Latvia precedent)

Slovakia context: Visegrad Group member (V4 with Czech Republic + Poland
