"""
SSI Pipeline — Slovakia L1 connector: OSM Overpass API.

Source ID:   SK-C1-osm-overpass
...
DIFFERENCE FROM LATVIA CONNECTOR (post-Convention #78 BINDING first-enforcement test):
  - COMPACT SCALE — Slovakia is ~49k km² (similar to Estonia).
    Single-query per element type is safe; no bbox partitioning needed.

  - REGION-JURISDICTION × VOLTAGE-CLASS OWNER FALLBACK — 7th cohort-wide
    application (after Belgium + Netherlands + Chile + Hungary + Slovenia
    + Colombia + Norway). SEPS TSO ≥220 kV (750/420/400/220 kV EHV backbone)
    + 3 regional DSOs (ZSD west + SSD centre + VSD east) via NUTS-3 → DSO
    map (Slovakia's clean 8-code partition). 110 kV MIXED tier defaults
    to SEPS. Private infrastructure (Slovak Railways electric traction,
    U.S. Steel Košice + Slovnaft industrial captives) surfaces via direct
    OSM operator= tag.

  - SLOVAK NFC + CYRILLIC + TYPOGRAPHIC-QUOTE UNICODE ALIAS NORMALISATION
    per Convention #78 BINDING enforcement — Unicode NFC + case-insensitive
    lookup for Slovak diacritics (č š ž ď ľ ň ť ĺ ŕ á í ó ú ý) + Cyrillic
    (сепс / зсд / ссд / всд — Rusyn/Ukrainian minority OSM contributors
    in eastern Slovakia bordering Ukraine) + Slovak typographic quotes
    („..." like Czech/German/Latvian). FIRST empirical test post-BINDING
    promotion (Latvia Priority 18 closure, 16 July 2026). Preserves
    original tag in raw_attributes.osm_original_operator for audit trail.

  - TSO THRESHOLD 220 kV — higher than Baltic Trio 110 kV due to Slovakia's
    63% baseline concentration at 110 kV MIXED tier (SEPS + DSO overlap).
    750 kV Mir/Slavia Ukraine cross-border HVAC unique cohort-wide (Ukraine
    grid disconnection Feb 2025 kept infrastructure intact). SEPS operates
    Continental European 400 kV EHV backbone + 220 kV Soviet-era HV.
    Slovakia is Visegrád Group EU-synchronised pre-2025 — NO EU sync
    event unlike Baltic Trio.
...
Slovakia's national territory defined by slovakia/bounds.json polygon.
...
# ── Query builders ──────────────────────────────────────────────────────
_SK_AREA_HEADER = 'area["ISO3166-1"="SK"]->.sk'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_SK_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.sk);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Slovakia.

    Single-query safe for compact country — Slovakia bbox ~22k km²
    (~1.5× Lithuania, ~2× Slovenia).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_SK_AREA_HEADER};'
        f'(way["power"="line"](area.sk);'
        f'way["power"="cable"](area.sk);'
        f'way["power"="minor_line"](area.sk););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Slovakia-specific: direct OSM operator= tag propagation with Slovak
    NFC + Cyrillic + typographic-quote Unicode alias normalisation per
    Convention #78 BINDING enforcement; if untagged, region-jurisdiction ×
    voltage-class resolver (SEPS ≥220 kV OR NUTS-3 → DSO OR SEPS 110 kV
    catch-all).
    """
...
def fetch(
    *,
    ...
    """Fetch OSM Slovakia substations + lines (single-query per element type).

    Compact country — no partitioning needed. Uses OSM operator= tag with
    Slovak NFC + Cyrillic + typographic-quote Unicode alias normalisation
    per Convention #78 BINDING enforcement, then region-jurisdiction ×
    voltage-class fallback (SEPS ≥220 kV OR NUTS-3 → DSO OR SEPS 110 kV
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Slovakia polygon "
    ...
    if result.substations:
        ...
        seps_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_fallback_SEPS")
        )
        dso_nuts3 = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_fallback_ZSD") or k.startswith("region_jurisdiction_fallback_SSD") or k.startswith("region_jurisdiction_fallback_VSD")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d SEPS_TSO + %d DSO_via_NUTS3",
            direct, alias_normalised, seps_tso, dso_nuts3,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} alias-normalised incl. Slovakian NFC + Cyrillic); "
            f"{seps_tso} SEPS_TSO + {dso_nuts3} DSO_via_NUTS3 "
            f"({100 * (seps_tso + dso_nuts3) / len(result.substations):.1f}%)."
        )
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass SK L1 connector — Slovakia region-jurisdiction × voltage-class via SEPS + 3 DSO NUTS-3 map + Convention #78 BINDING first-enforcement test")
