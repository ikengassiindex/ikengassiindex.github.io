"""
SSI Pipeline — Slovakia v4.23 federation merger.

Merges OSM Overpass L1 ingestion into slovakia/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Slovak NFC + Cyrillic + typographic-
     quote Unicode alias normalisation per Convention #78 BINDING
     enforcement) OR region-jurisdiction × voltage-class fallback per
     _base.py::resolve_owner_from_region_jurisdiction (SEPS TSO ≥220 kV
     OR NUTS-3 → DSO map OR SEPS 110 kV catch-all). Slovakia baseline:
     0% owner tagged. Expected post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 100% coverage — PERFECT
     — no fill opportunity; Slovakia baseline is voltage-tag-complete).
...
OSM tag (including Slovakian NFC diacritics and Cyrillic script) in
...
SSI_DATA_JSON = REPO_ROOT / "slovakia" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "slovakia" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "slovakia" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "SK_v43_") -> str:
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (slovakia/ssi-data.json, prior vintage)",
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Slovakia (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Slovakia v4.23 federation merger")
