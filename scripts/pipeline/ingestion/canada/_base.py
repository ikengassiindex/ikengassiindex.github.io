"""
SSI Pipeline — Canada v4.3 ingestion, shared base layer.
...
def emit_audit_sidecar(
    ...
    *,
    ...
    parent_preflight_yaml: str = "canada/v4_3-ingestion-audit-canada-preflight.yaml",
) -> Path:
    """Write per-source audit YAML sidecar per §5ter state-transition contract.

    Filename convention: v4_3-ingestion-audit-canada-<source_slug>.yaml
    (e.g. v4_3-ingestion-audit-canada-canvec-resmgt.yaml).
    ...
    out_path = output_dir / f"v4_3-ingestion-audit-canada-{slug}.yaml"

    payload_lines = [
        "# SSI Index v4.3 workstream — Canada ingestion fetch audit",
        "# Auto-generated by scripts/pipeline/ingestion/canada/_base.py::emit_audit_sidecar",
        f"# Parent pre-flight: {parent_preflight_yaml}",
        "",
        f"schema_version: v4_3-ingestion-audit-fetch-1",
