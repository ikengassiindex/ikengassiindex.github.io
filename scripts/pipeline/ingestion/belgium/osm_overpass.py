"""
SSI Pipeline — Belgium L1 connector: OSM Overpass API.
...
DIFFERENCE FROM AUSTRALIA CONNECTOR:
  - COMPACT SCALE — Belgium is 30k km² (~1/250 Australia). Single-query
...
# ── Query builders ──────────────────────────────────────────────────────
_BE_AREA_HEADER = 'area["ISO3166-1"="BE"]->.be'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_BE_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.be);'
        f'out center tags;'
    )
...
def _query_lines() -> str:
    ...
    Single-query safe for small country — Belgium bbox 30k km².
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_BE_AREA_HEADER};'
        f'(way["power"="line"](area.be);'
        f'way["power"="cable"](area.be);'
        f'way["power"="minor_line"](area.be););'
        f'out geom tags;'
    )
...
def fetch(
    *,
    ...
    """Fetch OSM Belgium substations + lines (single-query per element type).
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Belgium polygon "
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass BE L1 connector")
