"""
SSI Pipeline — Ireland v4.23 federation merger.

Merges OSM Overpass L1 ingestion into ireland/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with English + Gaeilge legal-form
     variants (Ltd/Limited/plc/DAC/Teoranta/Teo/Cuideachta) + Gaeilge
     diacritics (á é í ó ú + Éirid) + ESB Distribution pre-2010
     predecessor Unicode alias normalisation per Convention #78 BINDING
     7th enforcement) OR voltage-class × single-DSO fallback per
     _base.py::resolve_owner_from_region_jurisdiction (EirGrid
     ≥110 kV OR ESB Networks <110 kV OR ESB Networks default). Ireland
     baseline: 0.0% owner tagged (empirical audit — clean slate
     cohort-wide signature).
     Expected post-merge: ~100% (SIMPLEST cohort-wide via single-DSO).
  ...
  4b. Step 4b — Retroactive owner attribution on baseline subs without
      ...
      voltage-class × single-DSO resolver to close 100% owner coverage.
...
OSM tag (including English + Gaeilge legal-form variants (Ltd/Limited/
plc/DAC/Teoranta/Teo/Cuideachta), Gaeilge diacritics (á é í ó ú + Éirid),
and ESB Distribution pre-2010 predecessor legacy) in
...
SSI_DATA_JSON = REPO_ROOT / "ireland" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "ireland" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "ireland" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "IE_v43_") -> str:
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply voltage-class × single-DSO resolver to baseline subs without OSM match.
    ...
    Irish simplification: single national DSO makes retroactive attribution
    trivial — voltage-class threshold + single-DSO default. Every unmatched
    baseline sub gets attributed (100% coverage post-4b). Analogous to
    Greek P22 DEDDIE simplification.
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        # NUTS-3 for API-compat (Irish resolver empirically expects None; forward-compat)
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "(500m proximity miss). Same voltage-class × single-DSO resolver "
                    "as OSM path (EirGrid ≥110 kV OR ESB Networks <110 kV OR "
                    "ESB Networks default). Codified in Costa Rica Priority 13 "
                    "closure; Irish simplification via single national DSO ESB "
                    "Networks makes retroactive attribution trivial (Greek DEDDIE "
                    "precedent)."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (ireland/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="IE_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Ireland (subs + lines one-shot; single-query pattern)...")
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via voltage-class × single-DSO resolver",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Ireland v4.23 federation merger")
