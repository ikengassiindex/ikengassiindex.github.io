"""
SSI Pipeline — Greece v4.23 federation merger.

Merges OSM Overpass L1 ingestion into greece/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Greek script + Latin transliteration
     + English acronyms + ΑΕ/SA legal-form + Greek diacritics + DEI/PPC
     pre-2011 predecessor Unicode alias normalisation per Convention #78
     BINDING 4th enforcement) OR voltage-class × single-DSO fallback per
     _base.py::resolve_owner_from_region_jurisdiction (ADMIE ≥66 kV OR
     DEDDIE <66 kV OR DEDDIE default). Greece baseline: TBD% owner tagged.
...
OSM tag (including Greek script ΑΔΜΗΕ/ΔΕΔΔΗΕ/ΔΕΗ, Greek diacritics
(τόνος + διαλυτικά), Latin transliteration ADMIE/DEDDIE/DEI, English
acronyms IPTO/HEDNO/PPC, ΑΕ/SA legal-form variants, and DEI/PPC
pre-2011 predecessor legacy) in raw_attributes.osm_original_operator
for audit trail.
"""
...
SSI_DATA_JSON = REPO_ROOT / "greece" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "greece" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "greece" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "GR_v43_") -> str:
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    ...
    Greek simplification: single national DSO makes retroactive attribution
    trivial — voltage-class threshold + single-DSO default. Every unmatched
    baseline sub gets attributed (100% coverage post-4b).
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        # NUTS-3 for API-compat (Greek resolver ignores it — single-DSO)
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "as OSM path (ADMIE ≥66 kV OR DEDDIE <66 kV OR DEDDIE default). "
                    "Codified in Costa Rica Priority 13 closure; Greek simplification "
                    "makes retroactive attribution trivial via single national DSO."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (greece/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="GR_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Greece (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Greece v4.23 federation merger")
