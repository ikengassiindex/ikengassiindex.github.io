"""
SSI Pipeline — Lithuania L1 connector: OSM Overpass API.

Source ID:   LT-C1-osm-overpass
...
DIFFERENCE FROM COSTA RICA + LUXEMBOURG CONNECTORS:
  - COMPACT SCALE — Lithuania is ~22k km² (~1.1× Slovenia, ~30× Luxembourg).
    Single-query per element type is safe; no bbox partitioning needed.

  - PURE-MONOPOLY OWNER FALLBACK — IEC (Lithuania Electric Corporation)
    handles ~99% of national grid. No territorial DSO geofence needed;
    private IPPs (OPC / Dorad / Delek / Enlight) surface via direct OSM
    operator= tag. Industrial captives (Ashdod Refineries / Lithuania Chemicals)
    similarly surface via direct tag.

  - HEBREW UNICODE ALIAS NORMALISATION — Unicode NFC + case-insensitive
    lookup for חברת חשמל לישראל / חשמל / נגה / דורד etc. Preserves original
    Hebrew tag in raw_attributes.osm_original_operator for audit trail.

  - UNIQUELY LOW TSO THRESHOLD at 161 kV — Lithuania's national HV trunk.
...
Lithuania's national territory defined by lithuania/bounds.json polygon.
...
# ── Query builders ──────────────────────────────────────────────────────
_LT_AREA_HEADER = 'area["ISO3166-1"="LT"]->.lt'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_LT_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.lt);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Lithuania.

    Single-query safe for compact country — Lithuania bbox ~22k km²
    (~3× Slovenia, 30× Luxembourg).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_LT_AREA_HEADER};'
        f'(way["power"="line"](area.lt);'
        f'way["power"="cable"](area.lt);'
        f'way["power"="minor_line"](area.lt););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Lithuania-specific: direct OSM operator= tag propagation with Hebrew
    Unicode alias normalisation; if untagged, pure monopoly-default
    fallback (IEC via Layer 1 TSO threshold OR Layer 2 monopoly default).
    """
...
def fetch(
    *,
    ...
    """Fetch OSM Lithuania substations + lines (single-query per element type).

    Compact country — no partitioning needed. Uses OSM operator= tag with
    Hebrew Unicode alias normalisation, then pure monopoly-default fallback
    (Litgrid via Layer 1 TSO threshold ≥110 kV OR Layer 2 monopoly default)
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Lithuania polygon "
    ...
    if result.substations:
        ...
        monopoly_litgrid_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_Litgrid_TSO_threshold")
        )
        monopoly_eso_default = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_ESO_default")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d Litgrid_TSO_threshold + %d ESO_default",
            direct, alias_normalised, monopoly_litgrid_tso, monopoly_eso_default,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} alias-normalised incl. Hebrew Unicode); "
            f"{monopoly_litgrid_tso} Litgrid_TSO_threshold + {monopoly_eso_default} ESO_default "
            f"({100 * (monopoly_litgrid_tso + monopoly_eso_default) / len(result.substations):.1f}%)."
        )
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass IL L1 connector")
