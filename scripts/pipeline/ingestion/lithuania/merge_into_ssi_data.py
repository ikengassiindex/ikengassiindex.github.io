"""
SSI Pipeline — Lithuania v4.23 federation merger.

Merges OSM Overpass L1 ingestion into lithuania/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Hebrew Unicode alias normalisation)
     OR pure monopoly-default fallback per _base.py::resolve_owner_from_2_operator_monopoly
     (IEC TSO threshold ≥161 kV OR IEC monopoly default).
     Lithuania baseline: 0% owner tagged. Expected post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 100% at 161+400 kV transmission —
     LARGE MULTI-TIER FILL OPPORTUNITY from OSM MV distribution).
...
OSM tag (including Hebrew Unicode) in raw_attributes.osm_original_operator
for audit trail.
"""
...
SSI_DATA_JSON = REPO_ROOT / "lithuania" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "lithuania" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "lithuania" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "LT_v43_") -> str:
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (lithuania/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="LT_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Lithuania (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Lithuania v4.23 federation merger")
