"""
SSI Pipeline — Colombia v4.23 federation merger.

Merges OSM Overpass L1 ingestion into colombia/ssi-data.json + grid-geo.json.

Merger operations:

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with alias normalisation) OR department-map
     + ISA default fallback per _base.py::resolve_owner_from_department_map.
     Colombia baseline: 0% owner tagged, 100% region tagged as department names.
     Expected post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 100% at 110/115/220/230/500 kV
     transmission tier — LARGE MULTI-TIER FILL OPPORTUNITY from OSM MV).

  4. Net-new substations with L2/L3 null placeholders (Convention #56).

  4b. Step 4b — Retroactive owner attribution on baseline subs without OSM
      match. USES BASELINE DEPARTMENT FIELD (100% populated) — cleanest
      retroactive path yet cohort-wide.

  5. Line densification: append + dedupe by midpoint 100m + voltage ±20%.

  6. Audit trail with owner_provenance + department_source flag.

Convention #56 visibly-honest: alias normalisation preserves original OSM
tag in raw_attributes.osm_original_operator; department source (baseline
vs OSM tag) tracked in v43_provenance sidecar.
"""
...
SSI_DATA_JSON = REPO_ROOT / "colombia" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "colombia" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "colombia" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "CO_v43_") -> str:
...
def merge_substations(
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            # Special enrichment path: if OSM owner is ISA-default but baseline
            # has a department, promote the baseline department to a proper DSO
            baseline_dept = existing_sub.get("region")
            if (
                owner_provenance == "region_jurisdiction_fallback_ISA_default_state_utility"
                and baseline_dept
            ):
                # Re-run resolver with baseline department (OSM tag missing)
                v = ov.voltage_kv if ov.voltage_kv and ov.voltage_kv > 0 else None
                promoted_owner, promoted_prov = resolve_owner_from_department_map(
                    v, baseline_dept, float(ov.latitude), float(ov.longitude)
                )
                if promoted_owner and promoted_owner != "ISA":
                    owner = promoted_owner
                    owner_provenance = f"osm_matched_baseline_dept_promoted_{promoted_prov}"
            ...
            v43_prov[SOURCE_ID] = {
                ...
                "department_source": "baseline" if baseline_dept else "osm",
                "department": baseline_dept or ov.raw_attributes.get("department_detected"),
            }
            ...
            new_sub = {
                ...
                "osm_feature_id": ov.feature_id,
                "osm_substation_subtype": ov.raw_attributes.get("osm_substation_subtype"),
                "region": ov.raw_attributes.get("department_detected"),
                "province": None,
                "version": "4.2",
                ...
                "v43_provenance": {SOURCE_ID: {
                    ...
                    "osm_substation_subtype": ov.raw_attributes.get("osm_substation_subtype"),
                    "osm_original_operator": ov.raw_attributes.get("osm_original_operator"),
                    "department_detected": ov.raw_attributes.get("department_detected"),
                }},
...
# ── Step 4b: Retroactive attribution using baseline department field ─────
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply department-map + ISA default resolver to baseline subs without OSM match.

    Colombia-specific: baseline 100% region-populated with department names →
    cleanest retroactive path yet. Every unmatched sub resolves via department
    map with ISA state-utility default catch-all.
    """
    retroactive_count = 0
    ...
    for s in existing_subs:
        ...
        dept = s.get("region")
        owner, provenance = resolve_owner_from_department_map(
            v, dept, float(lat), float(lon)
        )
        if owner:
            s["owner"] = owner
            ...
            v43_prov[retro_source_id] = {
                ...
                "department_source": "baseline" if dept else "none",
                "department": dept,
                "note": (
                    "Applied post-merge Step 4b to baseline subs without OSM match. "
                    "Uses baseline department field (100% populated) → clean DSO map. "
                    "Codified in Costa Rica Priority 13, refined in Israel Priority 14."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (colombia/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="CO_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Colombia (subs + lines one-shot; single-query pattern)...")
    ...
    # Step 4b — retroactive owner attribution using baseline department field
    retro_count, retro_breakdown = step_4b_retroactive_attribution(existing_subs)
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via department-map + ISA default",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Colombia v4.23 federation merger")
