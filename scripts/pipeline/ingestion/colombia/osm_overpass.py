"""
SSI Pipeline — Colombia L1 connector: OSM Overpass API.

Source ID:   CO-C1-osm-overpass
...
Feature classes ingested:
  - power=substation      expected 800-2500 elements
  - power=line             expected 3k-10k ways
  - power=cable            urban underground
  - power=minor_line       MV distribution branches

DIFFERENCE FROM SLOVENIA + ISRAEL + LUXEMBOURG CONNECTORS:
  - LARGE-SCALE SCOPE — Colombia is ~1,142k km² (50× Slovenia, 22× Israel).
    Single-query pattern per element type; if 504 gateway encountered
    may need bbox partitioning by department cluster.

  - REGION-JURISDICTION OWNER FALLBACK — 30+ non-ISA DSOs mapped by
    department name (Layer 2). Baseline has 100% region attribution as
    department names (28 distinct) — cleanest resolver base yet cohort-wide.

  - EMCALI GEOFENCE CARVE-OUT — Cali metro (3.30-3.60°N, -76.65 to -76.40°W)
    fires Emcali BEFORE Valle del Cauca department → EPSA lookup, ensuring
    Cali urban subs get correct attribution.

  - HISTORICAL ELECTRICARIBE ROUTING — legacy tag "Electricaribe" resolves
    by department (SW → Air-e, NE → Afinia) rather than by alias alone.
    Handled at merge layer where existing baseline region is authoritative.

  - SPANISH ACCENT NORMALISATION — Nariño / Bogotá / San Andrés / Chocó
    handled via _strip_accents() in _base.py.

Convention #56 visibly-honest degradation:
  - Missing OSM voltage tag → voltage_kv = None (not 0.0)
  - Missing operator tag → owner via department-map + ISA default fallback
  - Overpass 429/504 → exponential backoff; graceful degradation

Convention #60 non-commercial provenance:
  - OSM (ODbL) only. CREG + XM open-data cross-validation deferred.

Discipline #36 cross-border filter — 100m default tolerance (per _base).
5-country border zone (Panama + Ecuador + Peru + Brazil + Venezuela).
...
DEFAULT_TIMEOUT_SECS = 300  # larger for country-size fetch
DEFAULT_QUERY_TIMEOUT_SECS = 180
...
# ── Query builders ──────────────────────────────────────────────────────
_CO_AREA_HEADER = 'area["ISO3166-1"="CO"]->.co'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_CO_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.co);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Colombia.

    Single-query for large country — may need partitioning if 504 encountered.
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_CO_AREA_HEADER};'
        f'(way["power"="line"](area.co);'
        f'way["power"="cable"](area.co);'
        f'way["power"="minor_line"](area.co););'
        f'out geom tags;'
    )
...
def _extract_department(tags: dict) -> str | None:
    """Extract Colombian department from OSM tags if present.

    OSM tags for Colombia may carry: addr:state, region, department.
    Return the department name (lowercase-normalized) if match.
    """
    for key in ("addr:state", "is_in:state", "department", "region"):
        val = tags.get(key)
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Colombia-specific: direct OSM operator= tag propagation with alias
    normalisation; if untagged, department-map + ISA default fallback.
    """
    ...
    if op_tag:
        ...
        owner, owner_provenance = resolve_owner_from_department_map(
            voltage_kv, department, lat, lon
        )
...
def fetch(
    *,
    ...
    """Fetch OSM Colombia substations + lines (single-query per element type).

    Large country — may need bbox partitioning if 504 encountered. Uses OSM
    operator= tag with alias normalisation, then department-map fallback
    with ISA state-utility default for untagged. Convention #56 preserved.
    """
    ...
    # ── Lines (single-query, no partitioning by default) ──
    if ingest_lines:
        try:
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Colombia polygon "
    ...
    if result.substations:
        ...
        isa_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_fallback_ISA_TSO")
        )
        isa_default = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_fallback_ISA_default")
        )
        dept_dso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_fallback_")
            and "ISA_" not in k
            and "Emcali_cali" not in k
        )
        emcali_geofence = sum(
            v for k, v in prov_counter.items()
            if k.startswith("region_jurisdiction_fallback_Emcali_cali")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d ISA_TSO + %d ISA_default + %d dept_DSO + %d Emcali_geofence",
            direct, alias_normalised, isa_tso, isa_default, dept_dso, emcali_geofence,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%); "
            f"{isa_tso} ISA_TSO_threshold + {isa_default} ISA_default + "
            f"{dept_dso} dept_DSO_map + {emcali_geofence} Emcali_geofence "
            f"({100 * (isa_tso + isa_default + dept_dso + emcali_geofence) / len(result.substations):.1f}%)."
        )
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass CO L1 connector")
