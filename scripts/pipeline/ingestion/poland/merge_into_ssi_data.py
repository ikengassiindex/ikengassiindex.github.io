"""
SSI Pipeline — Poland v4.23 federation merger.

Merges OSM Overpass L1 ingestion into poland/ssi-data.json + grid-geo.json.
...
Merger operations (Option A per pre-flight):
  ...
  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Polish NFC + Cyrillic + typographic-
     quote + comma-separated legal-form + 5-DSO REBRAND-PREDECESSOR
     CASCADE Unicode alias normalisation per Convention #78 BINDING 3rd
     enforcement) OR region-jurisdiction × voltage-class fallback per
     _base.py::resolve_owner_from_region_jurisdiction (PSE TSO ≥220 kV
     OR NUTS-3 → 4-DSO OR Layer 3 geofence (Innogy Stoen Warsaw metro)
     OR PGE catch-all default LARGEST DSO). Poland baseline: 0% owner
     tagged. Expected post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 44.5% voltage coverage —
     rich Polish OSM voltage tags; OSM enrichment expected to fill
     remaining ~50-55%).
  ...
  4b. Step 4b — Retroactive owner attribution on baseline subs without
      ...
      monopoly-default resolver to close 100% owner coverage gap.
...
OSM tag (including Polish NFC diacritics ą ć ę ł ń ó ś ź ż, Cyrillic
script from Belarusian + Ukrainian minority OSM contributors,
typographic quotes „...", and 5-DSO REBRAND-PREDECESSOR CASCADES
including Innogy Stoen 3-GENERATION UNIQUE cohort-wide multi-generation
tracking) in raw_attributes.osm_original_operator for audit trail.

Poland-specific complexity vs Czechia:
  - 5 DSOs (vs Czechia 3): PGE + Tauron + Enea + Energa + Innogy Stoen
  - RICHER NUTS-3 coverage — Polish OSM populates NUTS-3 tags for
    territorial DSO attribution (Slovenia + Hungary precedent — NOT
    Slovakia/Czechia which required Layer 3 lat/lon geofence catch-all)
  - Only Warsaw metro requires Layer 3 lat/lon geofence (Innogy Stoen)
  - 4 additional predecessor cascades (PGE 4-regional + Tauron 2-variant
    + Enea 1-variant + Energa 2-variant)
  - PLUS Innogy Stoen 3-GENERATION cascade (unique cohort-wide multi-
    generation tracking)
  - Compact JSON output per Task #125 90 MB sentinel prevention
    (LARGER country → bigger files, must stay under threshold)
"""
...
SSI_DATA_JSON = REPO_ROOT / "poland" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "poland" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "poland" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "PL_v43_") -> str:
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    """Apply monopoly-default resolver to baseline subs without OSM match.
    ...
    for s in existing_subs:
        ...
        # NUTS-3 for retroactive attribution — extract from baseline sub if available
        # Polish OSM populates NUTS-3 richer than Slovakia/Czechia baseline
        nuts3 = s.get('nuts3') or s.get('nuts_3') or s.get('ref:nuts:3')
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    ...
                    "(500m proximity miss). Same monopoly-default resolver as OSM path. "
                    "Codified in Costa Rica Priority 13 closure."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (poland/ssi-data.json, prior vintage)",
    ...
    # Compact JSON discipline per Task #125 90 MB sentinel prevention
    VOLTAGE_XCHECK_JSON.write_text(json.dumps(payload, ensure_ascii=False))
    logger.info(
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        # Poland-specific: PL_v43_l_ prefix (fixes Czechia's SK_v43_l_
        # copy-paste bug that carried Slovakia prefix in Czechia output)
        new_id = _stable_id(
            str(ln.raw_attributes.get("osm_id") or ln.feature_id),
            prefix="PL_v43_l_",
        )
        new_line = {
            "i": new_id,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info(
        "Fetching OSM Poland (subs single-shot per element type; "
        "lines 6-macroregion partitioned per LARGER country scale)..."
    )
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via monopoly-default resolver",
        retro_count,
    )
    ...
    if not dry_run:
        # Compact JSON output per Task #125 90 MB sentinel prevention
        # (Poland's LARGER scale requires strict compaction to stay under
        # threshold at expected 32k-42k subs + 130k-250k lines)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Poland v4.23 federation merger — Convention #78 BINDING 3rd "
            "enforcement + Layer 3 geofence 3rd enforcement + "
            "Visegrád Trio completion milestone (3 of 3)"
        )
    )
    ...
    print()
    print("Poland v4.23 merge stats:")
