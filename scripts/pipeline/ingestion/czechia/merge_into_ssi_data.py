"""
SSI Pipeline — Czechia v4.23 federation merger.

Merges OSM Overpass L1 ingestion into czechia/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Czech NFC + Cyrillic + typographic-
     quote + comma-separated legal-form + E.ON→EG.D 2021 rebrand Unicode
     alias normalisation per Convention #78 BINDING 2nd enforcement) OR
     region-jurisdiction × voltage-class fallback per
     _base.py::resolve_owner_from_region_jurisdiction (ČEPS TSO ≥220 kV
     OR NUTS-3 → DSO OR Layer 3 geofence (PRE Prague metro + EG.D 2-region
     bbox + ČEZ Distribuce catch-all)). Czechia baseline: 0% owner tagged.
     Expected post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 71% unknown — largest cohort
     voltage-attribution gap; OSM enrichment expected to fill ~65-70%).
...
OSM tag (including Czech NFC diacritics ě š č ř ž ý á í é ú ů ň ť ď ó,
Cyrillic script, typographic quotes „...", and E.ON→EG.D 2021 rebrand
predecessor legacy) in raw_attributes.osm_original_operator for audit trail.
"""
...
SSI_DATA_JSON = REPO_ROOT / "czechia" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "czechia" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "czechia" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "CZ_v43_") -> str:
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (czechia/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="SK_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Czechia (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Czechia v4.23 federation merger")
