"""
SSI Pipeline — Estonia v4.23 federation merger.

Merges OSM Overpass L1 ingestion into estonia/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Estonian + Cyrillic Unicode alias
     normalisation) OR 2-operator monopoly-default fallback per
     _base.py::resolve_owner_from_2_operator_monopoly (Elering TSO
     threshold ≥110 kV OR Elektrilevi monopoly default via Eesti Energia
     Grupp state holding). Estonia baseline: 0% owner tagged. Expected
     post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 77.9% coverage — 136 subs
     unknown-voltage; OSM MV distribution expected to fill sparse Elektrilevi
     10 kV tier via Baltic OSM density hypothesis).
...
OSM tag (including Estonian NFC diacritics and Cyrillic script) in
...
SSI_DATA_JSON = REPO_ROOT / "estonia" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "estonia" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "estonia" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "EE_v43_") -> str:
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    ...
    for s in existing_subs:
        ...
        owner, provenance = resolve_owner_from_2_operator_monopoly(v, float(lat), float(lon))
        if owner:
            s["owner"] = owner
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (estonia/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="EE_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Estonia (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Estonia v4.23 federation merger")
