"""
SSI Pipeline — Estonia L1 connector: OSM Overpass API.

Source ID:   EE-C1-osm-overpass
...
DIFFERENCE FROM LITHUANIA CONNECTOR (Baltic Trio sibling):
  - COMPACT SCALE — Estonia is ~45k km² (~1.5× Lithuania).
    Single-query per element type is safe; no bbox partitioning needed.

  - 2-OPERATOR MONOPOLY OWNER FALLBACK — Elering AS (state TSO ≥110 kV,
    2010 spinoff from Eesti Energia) + Elektrilevi OÜ (unified state DSO
    ≤110 kV, 2013 spinoff) handle 99%+ of national grid via Eesti Energia
    Grupp state holding. No territorial DSO geofence needed; private
    infrastructure (Estonian Railways electric traction, Alexela retail/
    generation IPPs) surfaces via direct OSM operator= tag.

  - ESTONIAN + CYRILLIC UNICODE ALIAS NORMALISATION — Unicode NFC +
    case-insensitive lookup for Estonian diacritics (Põhivõrk / Jaotusvõrk)
    plus Cyrillic (Элеринг / Электрилеви) from Ida-Virumaa Russian-speaking
    OSM contributors per Convention #78 sub-convention. Preserves original
    tag in raw_attributes.osm_original_operator for audit trail.

  - TSO THRESHOLD 110 kV — matches Lithuania (Baltic Trio pattern), higher
    than Israel (uniquely low 161 kV). Elering operates 450 kV DC (Estlink
    HVDC) + 330 kV (Soviet-era EHV) + 110 kV backbone.
...
Estonia's national territory defined by estonia/bounds.json polygon.
...
# ── Query builders ──────────────────────────────────────────────────────
_EE_AREA_HEADER = 'area["ISO3166-1"="EE"]->.ee'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_EE_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.ee);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Estonia.

    Single-query safe for compact country — Estonia bbox ~22k km²
    (~1.5× Lithuania, ~2× Slovenia).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_EE_AREA_HEADER};'
        f'(way["power"="line"](area.ee);'
        f'way["power"="cable"](area.ee);'
        f'way["power"="minor_line"](area.ee););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Estonia-specific: direct OSM operator= tag propagation with Estonian
    + Cyrillic Unicode alias normalisation; if untagged, 2-operator
    monopoly-default fallback (Elering via Layer 1 TSO threshold ≥110 kV
    OR Elektrilevi via Layer 2 monopoly default).
    """
...
def fetch(
    *,
    ...
    """Fetch OSM Estonia substations + lines (single-query per element type).

    Compact country — no partitioning needed. Uses OSM operator= tag with
    Estonian + Cyrillic Unicode alias normalisation, then 2-operator
    monopoly-default fallback (Elering via Layer 1 TSO threshold ≥110 kV
    OR Elektrilevi via Layer 2 monopoly default) for untagged.
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Estonia polygon "
    ...
    if result.substations:
        ...
        monopoly_elering_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_Elering_TSO_threshold")
        )
        monopoly_elektrilevi_default = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_Elektrilevi_default")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d Elering_TSO_threshold + %d Elektrilevi_default",
            direct, alias_normalised, monopoly_elering_tso, monopoly_elektrilevi_default,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} alias-normalised incl. Estonian NFC + Cyrillic); "
            f"{monopoly_elering_tso} Elering_TSO_threshold + {monopoly_elektrilevi_default} Elektrilevi_default "
            f"({100 * (monopoly_elering_tso + monopoly_elektrilevi_default) / len(result.substations):.1f}%)."
        )
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass EE L1 connector — Estonia 2-operator monopoly via Eesti Energia holding")
