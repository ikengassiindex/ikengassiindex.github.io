"""
SSI Pipeline — Latvia L1 connector: OSM Overpass API.

Source ID:   LV-C1-osm-overpass
...
Feature classes ingested:
  ...
  - power=line             expected 3k-7k ways
  - power=cable            urban underground
  - power=minor_line       MV distribution branches

DIFFERENCE FROM ESTONIA + LITHUANIA CONNECTORS (Baltic Trio 3rd instance):
  - COMPACT SCALE — Latvia is ~65k km² (~1.4× Estonia, ~same as Lithuania).
    Single-query per element type is safe; no bbox partitioning needed.

  - 2-OPERATOR MONOPOLY OWNER FALLBACK — AS Augstsprieguma tīkls (AST,
    state TSO ≥110 kV, 2011 spinoff from Latvenergo Transmission) + AS
    Sadales tīkls (unified state DSO ≤110 kV, 2007 rename from Sadales
    tīkli) handle 99%+ of national grid via Latvenergo Group state holding.
    No territorial DSO geofence needed; private infrastructure (Latvian
    Railways electric traction, Ventspils Nafta oil-terminal + Liepājas
    Metalurgs steel industrial captives) surfaces via direct OSM
    operator= tag.

  - LATVIAN + CYRILLIC UNICODE ALIAS NORMALISATION — Unicode NFC +
    case-insensitive lookup for Latvian diacritics (ā ē ī ō ū č ģ ķ ļ ņ
    š ž — Augstsprieguma tīkls / Sadales tīkls / Latvijas dzelzceļš) plus
    Cyrillic (аст / садалес тиклс / латвэнерго / латвияс электростацияс)
    from Latgale Russian-speaking OSM contributors — DEEPER cohort than
    Estonia's Ida-Virumaa (17.5% baseline share vs 14.3%) per Convention
    #78 sub-convention. Preserves original tag in
    raw_attributes.osm_original_operator for audit trail.

  - TSO THRESHOLD 110 kV — matches Lithuania + Estonia (Baltic Trio
    pattern). NO 450 kV HVDC (unlike Estonia's Estlink 1+2 to Finland).
    AST operates 330 kV (Soviet-era EHV) + 110 kV backbone. LatEst 330 kV
    AC cross-border tie to Estonia; NordBalt HVDC routes via Lithuania
    to Sweden (not directly through Latvia). Baltic Trio 3rd instance —
    Convention #78 sub-convention BINDING promotion candidate at closure.
...
Latvia's national territory defined by latvia/bounds.json polygon.
...
# ── Query builders ──────────────────────────────────────────────────────
_LV_AREA_HEADER = 'area["ISO3166-1"="LV"]->.lv'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_LV_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.lv);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Latvia.

    Single-query safe for compact country — Latvia bbox ~22k km²
    (~1.5× Lithuania, ~2× Slovenia).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_LV_AREA_HEADER};'
        f'(way["power"="line"](area.lv);'
        f'way["power"="cable"](area.lv);'
        f'way["power"="minor_line"](area.lv););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Latvia-specific: direct OSM operator= tag propagation with Latvian
    + Cyrillic Unicode alias normalisation; if untagged, 2-operator
    monopoly-default fallback (AST via Layer 1 TSO threshold ≥110 kV
    OR Sadales tīkls via Layer 2 monopoly default).
    """
    ...
    if op_tag:
        ...
        owner, owner_provenance = resolve_owner_from_2_operator_monopoly(voltage_kv, lat, lon)
...
def fetch(
    *,
    ...
    """Fetch OSM Latvia substations + lines (single-query per element type).

    Compact country — no partitioning needed. Uses OSM operator= tag with
    Latvian + Cyrillic Unicode alias normalisation, then 2-operator
    monopoly-default fallback (AST via Layer 1 TSO threshold ≥110 kV
    OR Sadales tīkls via Layer 2 monopoly default) for untagged.
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Latvia polygon "
    ...
    if result.substations:
        ...
        monopoly_ast_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_AST_TSO_threshold")
        )
        monopoly_sadales_tikls_default = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_Sadales_tikls_default")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d AST_TSO_threshold + %d Sadales_tikls_default",
            direct, alias_normalised, monopoly_ast_tso, monopoly_sadales_tikls_default,
        )
        result.warnings.append(
            f"Owner enrichment: {direct}/{len(result.substations)} direct "
            f"OSM operator= tag ({100 * direct / len(result.substations):.1f}%; "
            f"of these {alias_normalised} alias-normalised incl. Latvian NFC + Cyrillic); "
            f"{monopoly_ast_tso} AST_TSO_threshold + {monopoly_sadales_tikls_default} Sadales_tikls_default "
            f"({100 * (monopoly_ast_tso + monopoly_sadales_tikls_default) / len(result.substations):.1f}%)."
        )
...
def _cli_main() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="OSM Overpass LV L1 connector — Latvia 2-operator monopoly via Latvenergo holding (Baltic Trio 3rd instance)")
