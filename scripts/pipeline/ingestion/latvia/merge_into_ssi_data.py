"""
SSI Pipeline — Latvia v4.23 federation merger.

Merges OSM Overpass L1 ingestion into latvia/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Latvian + Cyrillic Unicode alias
     normalisation) OR 2-operator monopoly-default fallback per
     _base.py::resolve_owner_from_2_operator_monopoly (AST TSO
     threshold ≥110 kV OR Sadales tīkls monopoly default via Latvenergo
     Group state holding). Latvia baseline: 0% owner tagged. Expected
     post-merge: ~100%.

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 99.0% coverage — only
     12 subs unknown-voltage, minimal fill opportunity; Latvia baseline
     is voltage-tag-mature).
...
OSM tag (including Latvian NFC diacritics and Cyrillic script) in
...
SSI_DATA_JSON = REPO_ROOT / "latvia" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "latvia" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "latvia" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "LV_v43_") -> str:
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (latvia/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="LV_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Latvia (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Latvia v4.23 federation merger")
