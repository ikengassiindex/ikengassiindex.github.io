"""
SSI Pipeline — Israel L1 connector: OSM Overpass API.

Source ID:   IL-C1-osm-overpass
...
DIFFERENCE FROM COSTA RICA + LUXEMBOURG CONNECTORS:
  - COMPACT SCALE — Israel is ~22k km² (~1.1× Slovenia, ~10× Luxembourg).
    Single-query per element type is safe; no bbox partitioning needed.

  - PURE-MONOPOLY OWNER FALLBACK — IEC (Israel Electric Corporation)
    ...
    operator= tag. Industrial captives (Ashdod Refineries / Israel Chemicals)
  ...
  - UNIQUELY LOW TSO THRESHOLD at 161 kV — Israel's national HV trunk.
    Reflects US-standard heritage from 1920s-30s grid design.
...
Israel's national territory defined by israel/bounds.json polygon.
...
# ── Query builders ──────────────────────────────────────────────────────
_IL_AREA_HEADER = 'area["ISO3166-1"="IL"]->.il'
...
def _query_substations(feature_type: str = "way") -> str:
    ...
    return (
        ...
        f'{_IL_AREA_HEADER};'
        f'{feature_type}["power"="substation"](area.il);'
        f'out center tags;'
    )


def _query_lines() -> str:
    """Build Overpass query for all lines within Israel.

    Single-query safe for compact country — Israel bbox ~22k km²
    (~1.1× Slovenia, 10× Luxembourg).
    """
    return (
        f'[out:json][timeout:{DEFAULT_QUERY_TIMEOUT_SECS}];'
        f'{_IL_AREA_HEADER};'
        f'(way["power"="line"](area.il);'
        f'way["power"="cable"](area.il);'
        f'way["power"="minor_line"](area.il););'
        f'out geom tags;'
    )
...
def _substation_from_osm(el: dict) -> SubstationRecord | None:
    ...
    Israel-specific: direct OSM operator= tag propagation with Hebrew
    ...
    if op_tag:
        ...
        owner, owner_provenance = resolve_owner_from_monopoly_default(voltage_kv, lat, lon)
...
def fetch(
    *,
    ...
    """Fetch OSM Israel substations + lines (single-query per element type).
    ...
    (IEC via Layer 1 TSO threshold ≥161 kV OR Layer 2 monopoly default)
    ...
    if apply_bounds:
        ...
        if subs_dropped or lines_dropped:
            result.warnings.append(
                ...
                f"{len(lines_dropped)} lines outside Israel polygon "
    ...
    if result.substations:
        ...
        monopoly_ice_tso = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_IEC_TSO_threshold")
        )
        monopoly_ice_default = sum(
            v for k, v in prov_counter.items()
            if k.startswith("monopoly_fallback_IEC_default")
        )
        alias_normalised = prov_counter.get("osm_operator_tag_direct_alias_normalised", 0)
        logger.info(
            "Owner enrichment: %d direct OSM (%d alias-normalised) + %d IEC_TSO_threshold + %d IEC_default",
            direct, alias_normalised, monopoly_ice_tso, monopoly_ice_default,
        )
        result.warnings.append(
            ...
            f"{monopoly_ice_tso} IEC_TSO_threshold + {monopoly_ice_default} IEC_default "
            f"({100 * (monopoly_ice_tso + monopoly_ice_default) / len(result.substations):.1f}%)."
        )
