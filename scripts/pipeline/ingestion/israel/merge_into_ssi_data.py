"""
SSI Pipeline — Israel v4.23 federation merger.

Merges OSM Overpass L1 ingestion into israel/ssi-data.json + grid-geo.json.
...
Merger operations (Option A per pre-flight):
  ...
  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     ...
     OR pure monopoly-default fallback per _base.py::resolve_owner_from_monopoly_default
     (IEC TSO threshold ≥161 kV OR IEC monopoly default).
     Israel baseline: 0% owner tagged. Expected post-merge: ~100%.
...
SSI_DATA_JSON = REPO_ROOT / "israel" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "israel" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "israel" / "v4_23-voltage-cross-validation.json"
...
# ── Utilities ────────────────────────────────────────────────────────────
def _stable_id(payload: str, prefix: str = "IL_v43_") -> str:
...
def step_4b_retroactive_attribution(existing_subs: list[dict]) -> tuple[int, dict[str, int]]:
    ...
    for s in existing_subs:
        ...
        owner, provenance = resolve_owner_from_monopoly_default(v, float(lat), float(lon))
        if owner:
            s["owner"] = owner
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (israel/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="IL_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Israel (subs + lines one-shot; single-query pattern)...")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Israel v4.23 federation merger")
