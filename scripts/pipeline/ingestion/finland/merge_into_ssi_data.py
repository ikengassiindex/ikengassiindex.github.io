"""
SSI Pipeline — Finland v4.23 federation merger.

Merges OSM Overpass L1 ingestion into finland/ssi-data.json + grid-geo.json.

Merger operations (Option A per pre-flight):

  1. Owner-enrichment on matched pairs (500 m Discipline #41 threshold):
     Direct OSM operator= tag (with Finnish + Swedish + Sami trilingual
     cohabitation + diacritic composition (ä ö å + Sami ŋ ǯ â) + predecessor
     rebrand cascades (IVO→Fortum 1998 + Fortum→Caruna 2014 + Vattenfall
     Finland→Elenia 2012 + Helsingin Energia→Helen 2015) + nuclear
     generation-vs-distribution separation Unicode NFC alias normalisation
     per Convention #78 BINDING 11th enforcement post-DECADE-MILESTONE)
     OR 8-layer multi-DSO resolver per _base.py::
     resolve_owner_from_region_jurisdiction. Finland baseline: 0.0% owner
     tagged (CLEAN SLATE PATTERN — Ireland/NZ/DK analog).
     Expected post-merge: ~100% (Nordic cluster extension).

  2. Voltage cross-validation on matched pairs (>1.5× ratio flagged).

  3. Voltage fill on matched pairs (baseline 99%+ populated).
...
OSM tag (including Finnish legal-forms Oyj/Oy, Swedish/Åland Ab/Abp,
Finnish + Swedish diacritic composition ä ö å, Sami diacritics ŋ ǯ â,
nuclear generation-vs-distribution separation TVO/Fortum NOT DSO
flag-routing, IVO→Fortum 1998 + Fortum→Caruna 2014 + Vattenfall→
Elenia 2012 + Helsingin Energia→Helen 2015 predecessor cascades, and
Åland autonomous Kraftnät Åland Swedish-only jurisdiction) in
...
SSI_DATA_JSON = REPO_ROOT / "finland" / "ssi-data.json"
GRID_GEO_JSON = REPO_ROOT / "finland" / "grid-geo.json"
VOLTAGE_XCHECK_JSON = REPO_ROOT / "finland" / "v4_23-voltage-cross-validation.json"
...
def _stable_id(payload: str, prefix: str = "FI_v43_") -> str:
...
def _v42_placeholder_fields() -> dict[str, Any]:
    return {
        ...
        "R_P5": None, "R_P95": None, "Re_raw": 1.0, "Re_norm": 0.0,
        "add_sum": 0.0, "mult_product": 1.0, "P_critical": None,
        "CI_width": None, "component_alert": 0.0, "alert_components": [],
        "alert_flag": False, "classification": None, "confidence_tier": None,
        "fleet_percentile": None, "skewness": None, "environmental": {},
    }
    ...
    for ov in osm_subs:
        ...
        if best_idx is not None:
            ...
            v43_prov[SOURCE_ID] = {
                ...
                "osm_name": osm_name,
                "osm_name_fi": ov.raw_attributes.get("osm_name_fi"),
                "osm_name_sv": ov.raw_attributes.get("osm_name_sv"),
                "osm_name_se": ov.raw_attributes.get("osm_name_se"),
    ...
    for s in existing_subs:
        ...
        if owner:
            ...
            v43_prov[retro_source_id] = {
                ...
                "note": (
                    "Applied post-merge Step 4b to baseline subs without OSM "
                    "match (500m proximity miss). Same 8-layer multi-DSO "
                    "resolver as OSM path (Layer 1 Fingrid TSO ≥110 kV OR "
                    "Layer 2 VR Group 25 kV AC OR Layer 3a nuclear name-match "
                    "OR Layer 3b §4bis.5 Helsinki metro 3-way geofence 7TH "
                    "COHORT-WIDE ENFORCEMENT OR Layer 4a Åland Swedish OR "
                    "Layer 4b industrial captive OR Layer 5 region → dominant "
                    "DSO OR Layer 6 Fingrid catch-all). Codified in Costa "
                    "Rica Priority 13 closure; Finland Nordic cluster "
                    "extension with 6 major DSOs + Convention #78 BINDING "
                    "11th enforcement post-DECADE-MILESTONE."
                ),
            }
            retroactive_count += 1
...
def emit_voltage_cross_validation(findings: list[dict], total_matched: int) -> None:
    payload = {
        ...
        "source_pair": {
            "existing_source": "OSM voltage= tags (finland/ssi-data.json, prior vintage)",
...
def merge_lines(
    ...
    for ln in osm_lines:
        ...
        new_id = _stable_id(str(ln.raw_attributes.get("osm_id") or ln.feature_id), prefix="FI_v43_l_")
        new_line = {
            "i": new_id,
            "p": coords,
            "kv": kv,
...
def main(*, dry_run: bool = False, skip_lines: bool = False) -> dict:
    ...
    logger.info("Fetching OSM Finland (subs + lines one-shot; single-query pattern)...")
    ...
    logger.info(
        "Step 4b retroactive attribution: %d subs closed via 8-layer multi-DSO resolver",
        retro_count,
    )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Finland v4.23 federation merger")
