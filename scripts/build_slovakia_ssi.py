#!/usr/bin/env python3
"""
Build slovakia/ssi-data.json from grid-geo.json + data/slovakia_config.json
using scripts/score-country.py as the engine, then post-process to:
 ...
 - fix province from region code -> human name
 - add kreis (capital), kraj_name_sk
...
# Load inputs
CONFIG = json.load(open(REPO / 'data' / 'slovakia_config.json'))
GRID = json.load(open(REPO / 'slovakia' / 'grid-geo.json'))
...
for s in substations:
    ...
    s['kraj_name_sk'] = rcfg.get('name', region)
    ...
    unemp_raw = rcfg.get('unemp', 0.058)
    ...
    rd_base = {
        'SK010': 1.8, 'SK021': 0.7, 'SK022': 0.6, 'SK023': 0.7,
        'SK031': 0.9, 'SK032': 0.6, 'SK041': 0.5, 'SK042': 1.0,
    }.get(region, 0.93)
...
meta = {
    ...
    'generator': 'SSI v4.0.2 Pipeline (Slovakia, score-country.py + post-process)',
    'note': f'Substations from OpenStreetMap ({total} nodes), scored at kraj level (8 NUTS-3) with deterministic per-substation jitter. R3 tier 1.04 (DACH-Visegrad median); R6 seismic alpha 0.40 (Carpathian moderate); MIN_FLEET 1100.',
}
...
out_path = REPO / 'slovakia' / 'ssi-data.json'
