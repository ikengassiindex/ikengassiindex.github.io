...
def check_country(slug, repo_root="."):
    ...
    # Same defect as D#17: sharded manifests carry no "substations" key, so this
    # gate errored on the six largest fleets instead of checking them.
    try:
        import sys as _sys, pathlib as _pl
        _sys.path.insert(0, str(_pl.Path(__file__).resolve().parent.parent))
        from scripts._ssi_data_shard_reader import load_ssi_data
        d, subs, _sharded = load_ssi_data(slug)
    except Exception as e:
        return {"slug": slug, "status": "ERROR", "reason": f"{type(e).__name__}: {str(e)[:50]}"}
