...
_MODIFIER_RANGES = {
    ...
    #     REVERSED 21 September 2026 back to 1.30. The pipeline's 1.50 was
    #     the outer bound of a soft_clip that can never bind: the sigmoid in
    #     engine.compute_r3 asymptotes at range_hi = 1.30. Syncing to it in
    #     June propagated a misreading rather than a value.
    #   • R7_cyber ceiling 1.50 → 1.05 (was permissive; pipeline says 1.05).
    # Canonical v4.0.2 (5 modifiers)
  # 1.30, not 1.50: the sigmoid in engine.compute_r3 asymptotes at range_hi,
  # and the anchor 'a median catchment leaves the base score unchanged' holds
  # only at the midpoint of [0.70, 1.30]. The 21 Aug pin took 1.50 from a
  # soft_clip that never binds. See doctrine/RECOMMENDATION_R3_C_mult_alpha_to_omega.md
    "R3_C_mult":      (0.70, 1.30),
    "R4_F_topo":      (0.80, 1.35),
...
def validate_file(filepath: str) -> Tuple[List[str], List[str]]:
    ...
    if isinstance(data, dict) and data.get("substations_shards") and "substations" not in data:
        ...
        # NOTICE, not a warning. This says what the validator DID, not that
        # something may be wrong, and a confirmation on the warnings channel
        # trains a reader to skim warnings — which is how a real one gets
        # missed. The validator returns two channels, not three, so the
        # distinction is carried by an explicit prefix rather than by a
        # signature change that would ripple into pipeline/run.py.
        warnings.append(
            f"NOTICE: Convention #79 sharded storage — validated "
            f"{len(_shard_subs):,} substations across "
            f"{len(data['substations_shards'])} shard files"
        )
    ...
    # TWO BAND RULES ARE LIVE, and this gate used to know only one.
    #
    # Measured 21 September 2026 over all 622,104 published records
    # (scripts/check_band_rule_per_country.py): every jurisdiction is banded
    # purely by one rule and none mixes them. 11 jurisdictions and 483,706
    # assets use the PER-COUNTRY percentile rule; 28 and 138,398 use the
    # ABSOLUTE cutoffs. Both are now pinned in the judgement layer as
    # classification_rank and classification_absolute.
    #
    # Checking absolute cutoffs alone reported 30,015 Italian substations —
    # 72.04 per cent of the canonical reference country — as misclassified.
    # Italy is banded on percentiles and reproduces at 100.00 per cent under
    # that rule. The data was right and this gate was wrong.
    #
    # It now accepts EITHER rule and fails only where a record matches
    # NEITHER, which is the condition that actually means "misbanded". It also
    # reports which rule the file follows, so the gate surfaces the cohort
    # inconsistency instead of hiding it behind a false alarm.
    ...
    def _norm(r, p5, p95):
        """The record's position in its own country's distribution, or None."""
        if r is None or p5 is None or p95 is None or (p95 - p5) <= 0:
            return None
        return max(0.0, min(1.0, (r - p5) / (p95 - p5)))

    # Exact boundary ties. 92 records of 622,104 match neither rule because
    # the anchors are stored to 4 decimals while scoring used full precision;
    # all 92 sit within 4.06e-05 of a cutoff. A tie at a cutoff is a rounding
    # artefact, not a misbanding, and is counted separately rather than
    # absorbed silently.
    BOUNDARY_TOL = 1e-4
    ...
    n_skipped_pre_l3 = 0
    n_boundary = 0
    by_rule = {'absolute': 0, 'rank': 0, 'both': 0}
    ...
    for s in substations:
        ...
        got = s.get('classification')
        expected = _band(r)
        rn = _norm(r, s.get('_band_norm_R_P5'), s.get('_band_norm_R_P95'))
        expected_rank = _band(rn) if rn is not None else None
        ok_abs = (got == expected)
        ok_rank = (expected_rank is not None and got == expected_rank)
        if ok_abs and ok_rank:
            by_rule['both'] += 1
        elif ok_abs:
            by_rule['absolute'] += 1
        elif ok_rank:
            ...
            if rn is not None and min(abs(rn - c) for c in CUTS) <= BOUNDARY_TOL:
                n_boundary += 1
                continue
            misclassified += 1
    ...
    if n_boundary:
        warnings.append(
            f"NOTICE: CHECK 8: {n_boundary} substations sit within {BOUNDARY_TOL} of a "
            f"band cutoff after per-country normalisation and match neither "
            f"rule exactly — rounding ties between full-precision scoring and "
            f"4-decimal stored anchors, not misbanding")
    if by_rule['absolute'] and by_rule['rank']:
        warnings.append(
            f"CHECK 8: this file mixes band rules — {by_rule['absolute']} "
            f"records match only the absolute cutoffs and {by_rule['rank']} "
            f"only the per-country rank. Every jurisdiction measured on "
            f"21 September 2026 was pure; a mixture is new and should be "
            f"investigated before it is published")
    if n_skipped_pre_l3 > 0:
        warnings.append(
            f"NOTICE: CHECK 8: {n_skipped_pre_l3}/{len(substations)} substations "
